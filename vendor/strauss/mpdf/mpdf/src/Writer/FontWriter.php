...
use WOI\PDF\Vendor\Mpdf\TTFontFile;
		...
		foreach ($this->mpdf->fonts as $k => $font) {
			...
			if ($type === 'Type0') {  // Adobe CJK Fonts
				...
				if ($this->mpdf->PDFA || $this->mpdf->PDFX) {
					throw new \WOI\PDF\Vendor\Mpdf\MpdfException('Core fonts are not allowed in PDF/A1-b or PDFX/1-a files (Times, Helvetica, Courier etc.)');
				}
				...
				throw new \WOI\PDF\Vendor\Mpdf\MpdfException(sprintf('Unsupported font type: %s (%s)', $type, $name));
			}
		}
	}
