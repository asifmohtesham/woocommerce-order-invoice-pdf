...
use WOI\PDF\Vendor\Mpdf\Conversion\DecToAlpha;
use WOI\PDF\Vendor\Mpdf\Conversion\DecToCjk;
use WOI\PDF\Vendor\Mpdf\Conversion\DecToHebrew;
use WOI\PDF\Vendor\Mpdf\Conversion\DecToOther;
use WOI\PDF\Vendor\Mpdf\Conversion\DecToRoman;
	...
	public function open($attr, &$ahtml, &$ihtml)
	{
		...
		if ((isset($p['POSITION'])
			...
			if ($this->mpdf->inFixedPosBlock) {
				throw new \WOI\PDF\Vendor\Mpdf\MpdfException('Cannot nest block with position:fixed or position:absolute');
			}
		...
		if ($currblk['inner_width'] < $mw) {
			...
			// if ($currblk['inner_width'] < $mw) { throw new \WOI\PDF\Vendor\Mpdf\MpdfException("DIV is too narrow for text to fit!"); }
		}
