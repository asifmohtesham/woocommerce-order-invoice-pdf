 ...
 */
class Imb extends \WOI\PDF\Vendor\Mpdf\Barcode\AbstractBarcode implements \WOI\PDF\Vendor\Mpdf\Barcode\BarcodeInterface
{
	...
	public function __construct($code, $xDim, $gapWidth, $daft)
	{
		if (!function_exists('bcadd')) {
			throw new \WOI\PDF\Vendor\Mpdf\Barcode\BarcodeException('IMB barcodes require bcmath extension to be loaded.');
		}
	...
	private function init($code, $gapWidth, $daft)
	{
		...
		switch (strlen($routingCode)) {
			...
			case 11:
				...
				break;
			default:
				throw new \WOI\PDF\Vendor\Mpdf\Barcode\BarcodeException(sprintf('Invalid MSI routing code "%s"', $routingCode));
		}
