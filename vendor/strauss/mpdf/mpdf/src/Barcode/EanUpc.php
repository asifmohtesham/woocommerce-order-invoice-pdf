 ...
 */
class EanUpc extends \WOI\PDF\Vendor\Mpdf\Barcode\AbstractBarcode implements \WOI\PDF\Vendor\Mpdf\Barcode\BarcodeInterface
{
	...
	private function init($code, $length)
	{
		if (preg_match('/[\D]+/', $code)) {
			throw new \WOI\PDF\Vendor\Mpdf\Barcode\BarcodeException(sprintf('Invalid EAN UPC barcode value "%s"', $code));
		}
		...
		if ($upce) {
			...
			if ($invalidUpce) {
				throw new \WOI\PDF\Vendor\Mpdf\Barcode\BarcodeException('UPC-A cannot produce a valid UPC-E barcode');
			}
		}
