...
if ( ! class_exists( '\\WOI\\PDF\\Semaphore' ) ) :
...
class Semaphore {
	 ...
	 * @var string
	 */
	public const CLEANUP_TRANSIENT_KEY = 'woi_pdf_semaphore_cleanup_scheduled';
	 ...
	 * @var string
	 */
	protected static $option_prefix = 'woi_pdf_semaphore_lock_';
	 ...
	 * @var string
	 */
	protected static $legacy_option_prefix = 'updraft_lock_woi_pdf_';
	...
	public function log( string $message, string $level = 'info', array $context = array() ): void {
		...
		$logs_enabled = isset( WOI_PDF()->settings->debug_settings['semaphore_logs'] );
	...
	public static function is_cleanup_scheduled(): bool {
		...
		} elseif ( function_exists( 'woi_pdf_log_error' ) ) {
	...
	public static function schedule_semaphore_cleanup(): void {
		if ( ! self::is_cleanup_scheduled() ) {
			$frequency     = apply_filters( self::get_cleanup_hook_name() . '_frequency', ( (int) ( WOI_PDF()->settings->debug_settings['cleanup_days'] ?? 7 ) ) * DAY_IN_SECONDS );
	...
	public static function init_cleanup(): void {
		...
		if ( ! WOI_PDF()->dependencies_are_ready() ) {
			return;
		}
