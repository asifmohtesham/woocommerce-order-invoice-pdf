...
if ( ! class_exists( '\\WOI\\PDF\\Install' ) ) :
...
class Install {
	...
	****************************************************************************/
	...
	public function do_install() {
		...
		if ( ! WOI_PDF()->is_woocommerce_activated() ) {
			return;
		}

		$version_setting   = 'woi_pdf_version';
		$installed_version = get_option( $version_setting );

		// installed version lower than plugin version?
		if ( version_compare( $installed_version, WOI_PDF_VERSION, '<' ) ) {

			if ( ! $installed_version ) {
				try {
					$this->install();
				} catch ( \Throwable $th ) {
					woi_pdf_log_error( sprintf( "Plugin install procedure failed (version %s): %s", WOI_PDF_VERSION, $th->getMessage() ), 'critical', $th );
				}
			} else {
				try {
					$this->upgrade( $installed_version );
				} catch ( \Throwable $th ) {
					woi_pdf_log_error( sprintf( "Plugin upgrade procedure failed (updating from version %s to %s): %s", $installed_version, WOI_PDF_VERSION, $th->getMessage() ), 'critical', $th );
				}
			}

			// new version number
			update_option( $version_setting, WOI_PDF_VERSION );
		} elseif ( $installed_version && version_compare( $installed_version, WOI_PDF_VERSION, '>' ) ) {
			try {
				$this->downgrade( $installed_version );
			} catch ( \Throwable $th ) {
				woi_pdf_log_error( sprintf( "Plugin downgrade procedure failed (downgrading from version %s to %s): %s", $installed_version, WOI_PDF_VERSION, $th->getMessage() ), 'critical', $th );
			}
		...
		add_action( 'admin_init', array( WOI_PDF(), 'deactivate_legacy_addons') );
	}
	...
	protected function install() {
		...
		if ( ! WOI_PDF()->is_dependency_version_supported( 'php' ) ) {
			return;
		}

		// check if upgrading from versionless (1.4.14 and older)
		if ( get_option( 'woi_pdf_general_settings' ) ) {
		...
		$settings_defaults = array(
			'woi_pdf_settings_general' => array(
				'download_display'			=> 'display',
				'template_path'				=> WOI_PDF()->plugin_path() . '/templates/Simple',
			...
			'woi_pdf_documents_settings_invoice' => array(
				'enabled'					=> 1,
			...
			'woi_pdf_documents_settings_packing-slip' => array(
				'enabled'					=> 1,
				// 'display_billing_address'	=> '',
				// 'display_email'				=> '',
				// 'display_phone'				=> '',
			),
			'woi_pdf_settings_debug' => array(
				// 'legacy_mode'				=> '',
				// 'enable_debug'				=> '',
				// 'html_output'				=> '',
				// 'html_output'				=> '',
		...
		set_transient( 'woi_pdf_new_install', 'yes', DAY_IN_SECONDS * 2 );

		// schedule the yearly reset number action
		if ( ! empty( WOI_PDF()->settings ) && is_callable( array( WOI_PDF()->settings, 'schedule_yearly_reset_numbers' ) ) ) {
			WOI_PDF()->settings->schedule_yearly_reset_numbers();
		}
	}
	...
	protected function upgrade( $installed_version ) {
		...
		$font_path = WOI_PDF()->main->get_tmp_path( 'fonts' );

		// Check if tmp folder exists => if not, initialize
		if (
			! WOI_PDF()->file_system->is_dir( $tmp_base ) ||
			! WOI_PDF()->file_system->is_writable( $tmp_base ) ||
			! WOI_PDF()->file_system->is_dir( $font_path ) ||
			! WOI_PDF()->file_system->is_writable( $font_path )
		) {
			WOI_PDF()->main->init_tmp();
		}

		// To ensure fonts will be copied to the upload directory
		delete_transient( 'woi_pdf_subfolder_fonts_has_files' );
		...
		if ( $installed_version == 'versionless' || version_compare( $installed_version, '1.5.28', '<' ) ) {
			$template_settings   = get_option( 'woi_pdf_template_settings' );
			$next_invoice_number = isset( $template_settings['next_invoice_number'] ) ? $template_settings['next_invoice_number'] : '';
			update_option( 'woi_pdf_next_invoice_number', $next_invoice_number );
		}

		// 2.0-dev update: reorganize settings
		if ( $installed_version == 'versionless' || version_compare( $installed_version, '2.0-dev', '<' ) ) {
			$old_settings = array(
				'woi_pdf_general_settings'  => get_option( 'woi_pdf_general_settings' ),
				'woi_pdf_template_settings' => get_option( 'woi_pdf_template_settings' ),
				'woi_pdf_debug_settings'    => get_option( 'woi_pdf_debug_settings' ),
			);

			// combine invoice number formatting in array
			$old_settings['woi_pdf_template_settings']['invoice_number_formatting'] = array();
			$format_option_keys = array( 'padding', 'suffix', 'prefix' );
			foreach ( $format_option_keys as $format_option_key ) {
				if ( isset( $old_settings['woi_pdf_template_settings']["invoice_number_formatting_{$format_option_key}"] ) ) {
					$old_settings['woi_pdf_template_settings']['invoice_number_formatting'][ $format_option_key ] = $old_settings['woi_pdf_template_settings']["invoice_number_formatting_{$format_option_key}"];
				}
			}

			// convert abbreviated email_ids
			if ( isset( $old_settings['woi_pdf_general_settings']['email_pdf'] ) ) {
				foreach ( $old_settings['woi_pdf_general_settings']['email_pdf'] as $email_id => $value ) {
					if ( $email_id == 'completed' || $email_id == 'processing' ) {
						$old_settings['woi_pdf_general_settings']['email_pdf'][ "customer_{$email_id}_order" ] = $value;
						unset( $old_settings['woi_pdf_general_settings']['email_pdf'][ $email_id ] );
					}
				}
			}
			...
			if ( ! empty( $old_settings['woi_pdf_template_settings']['template_path'] ) ) {
				$template_path = str_replace( '\\', '/', $old_settings['woi_pdf_template_settings']['template_path'] );
				...
				$old_settings['woi_pdf_template_settings']['template_path'] = $template_path;
			}

			// map new settings to old
			$settings_map = array(
				'woi_pdf_settings_general' => array(
					'download_display' => array( 'woi_pdf_general_settings' => 'download_display' ),
					'template_path'    => array( 'woi_pdf_template_settings' => 'template_path' ),
					'currency_font'    => array( 'woi_pdf_template_settings' => 'currency_font' ),
					'paper_size'       => array( 'woi_pdf_template_settings' => 'paper_size' ),
					'header_logo'      => array( 'woi_pdf_template_settings' => 'header_logo' ),
					'shop_name'        => array( 'woi_pdf_template_settings' => 'shop_name' ),
					'shop_address'     => array( 'woi_pdf_template_settings' => 'shop_address' ),
					'footer'           => array( 'woi_pdf_template_settings' => 'footer' ),
					'extra_1'          => array( 'woi_pdf_template_settings' => 'extra_1' ),
					'extra_2'          => array( 'woi_pdf_template_settings' => 'extra_2' ),
					'extra_3'          => array( 'woi_pdf_template_settings' => 'extra_3' ),
				),
				'woi_pdf_documents_settings_invoice' => array(
					'attach_to_email_ids'      => array( 'woi_pdf_general_settings' => 'email_pdf' ),
					'display_shipping_address' => array( 'woi_pdf_template_settings' => 'invoice_shipping_address' ),
					'display_email'            => array( 'woi_pdf_template_settings' => 'invoice_email' ),
					'display_phone'            => array( 'woi_pdf_template_settings' => 'invoice_phone' ),
					'display_date'             => array( 'woi_pdf_template_settings' => 'display_date' ),
					'display_number'           => array( 'woi_pdf_template_settings' => 'display_number' ),
					'number_format'            => array( 'woi_pdf_template_settings' => 'invoice_number_formatting' ),
					'reset_number_yearly'      => array( 'woi_pdf_template_settings' => 'yearly_reset_invoice_number' ),
					'my_account_buttons'       => array( 'woi_pdf_general_settings' => 'my_account_buttons' ),
					'invoice_number_column'    => array( 'woi_pdf_general_settings' => 'invoice_number_column' ),
					'invoice_date_column'      => array( 'woi_pdf_general_settings' => 'invoice_date_column' ),
					'disable_free'             => array( 'woi_pdf_general_settings' => 'disable_free' ),
				),
				'woi_pdf_documents_settings_packing-slip' => array(
					'display_billing_address' => array( 'woi_pdf_template_settings' => 'packing_slip_billing_address' ),
					'display_email'           => array( 'woi_pdf_template_settings' => 'packing_slip_email' ),
					'display_phone'           => array( 'woi_pdf_template_settings' => 'packing_slip_phone' ),
				),
				'woi_pdf_settings_debug' => array(
					'enable_debug' => array( 'woi_pdf_debug_settings' => 'enable_debug' ),
					'html_output'  => array( 'woi_pdf_debug_settings' => 'html_output' ),
				),
			);
			...
			foreach ( $settings_map as $new_option => $new_settings_keys ) {
				...
				$enabled = array( 'woi_pdf_documents_settings_invoice', 'woi_pdf_documents_settings_packing-slip' );
		...
		if ( version_compare( $installed_version, '2.0-beta-2', '<' ) ) {
			$next_number = get_option( 'woi_pdf_next_invoice_number' );
			if ( ! empty( $next_number ) ) {
				$number_store = new \WOI\PDF\Documents\SequentialNumberStore( 'invoice_number' );
				$number_store->set_next( (int) $next_number );
			}
			// we're not deleting this option yet to make downgrading possible
			// delete_option( 'woi_pdf_next_invoice_number' ); // clean up after ourselves
		}
		...
		if ( version_compare( $installed_version, '2.11.5', '<' ) ) {
			if ( ! empty( WOI_PDF()->settings ) && is_callable( array( WOI_PDF()->settings, 'maybe_migrate_template_paths' ) ) ) {
				WOI_PDF()->settings->maybe_migrate_template_paths();
			}
		}
		...
		if ( version_compare( $installed_version, '2.12.2-dev-1', '<' ) ) {
			...
			foreach ( $documents as $document ) {
				...
				$method            = WOI_PDF()->settings->get_sequential_number_store_method();
				$table_name        = apply_filters( 'woi_pdf_number_store_table_name', woi_pdf_sanitize_identifier( "{$wpdb->prefix}woi_pdf_{$store_name}" ), $store_name, $method );
		...
		if ( version_compare( $installed_version, '3.7.0-beta-4', '<' ) ) {
			...
			$general_settings            = get_option( 'woi_pdf_settings_general', [] );
			$invoice_ubl_settings        = get_option( 'woi_pdf_documents_settings_invoice_ubl', [] );
			...
			foreach ( $settings_to_migrate as $setting => $type ) {
				...
				if ( ! empty( $update ) ) {
					...
					foreach ( $update as $type ) {
						switch ( $type ) {
							case 'general':
								update_option( 'woi_pdf_settings_general', $general_settings );
								break;
							case 'invoice_ubl':
								$invoice_ubl_settings['enabled'] = '1';
								update_option( 'woi_pdf_documents_settings_invoice_ubl', $invoice_ubl_settings );
								break;
						}
					}
				}
			...
			if ( ! empty( $legacy_ubl_tax_settings ) ) {
				update_option( 'woi_pdf_settings_ubl_taxes', $legacy_ubl_tax_settings );
			}

			// set transient to flush rewrite rules if pretty links are enabled
			if ( WOI_PDF()->endpoint->pretty_links_enabled() ) {
				set_transient( 'woi_pdf_flush_rewrite_rules', 'yes', HOUR_IN_SECONDS );
			}
		}

		// 3.9.5-beta-4: migrate UBL tax schemes/categories
		if ( version_compare( $installed_version, '3.9.5-beta-4', '<' ) ) {
			$ubl_tax_settings = get_option( 'woi_pdf_settings_ubl_taxes', array() );
			...
			if ( ! empty( $ubl_tax_settings ) ) {
				...
				update_option( 'woi_pdf_settings_ubl_taxes', $ubl_tax_settings );
			}
		}

		// 4.0.0-beta-3: Remove translatability from VAT and COC fields
		if ( version_compare( $installed_version, '4.0.0-beta-3', '<' ) ) {
			$general_settings = get_option( 'woi_pdf_settings_general', array() );
		...
		if ( version_compare( $installed_version, '4.5.3-pr1195.1', '<' ) ) {
			...
			if ( ! empty( $states_setting ) && ! empty( $countries_setting ) ) {
				...
				delete_option( 'woi_pdf_dismiss_shop_address_notice' );
			}
		}

		// 5.0.0-pr1149.1: migrate UBL tax settings to IPS EDI settings
		if ( version_compare( $installed_version, '5.0.0-pr1149.1', '<' ) ) {
			$ubl_settings = get_option( 'woi_pdf_settings_ubl_taxes', array() );

			if ( ! empty( $ubl_settings ) ) {
				if ( update_option( 'woi_pdf_edi_tax_settings', $ubl_settings ) ) {
					delete_option( 'woi_pdf_settings_ubl_taxes' );
				}
			}
			...
			if ( ! empty( $invoice_settings['ubl'] ) ) {
				...
				update_option( 'woi_pdf_edi_settings', $edi_settings );
				unset( $invoice_settings['ubl'] );
				update_option( 'woi_pdf_documents_settings_invoice', $invoice_settings );
			}
		}

		// 5.9.0-i1457.1: migrate EDI settings
		if ( version_compare( $installed_version, '5.9.0-i1457.1', '<' ) ) {
			$edi_settings = get_option( 'woi_pdf_edi_settings', array() );
		...
		if ( version_compare( $installed_version, '5.10.0-i1490.1', '<' ) ) {
			...
			if ( ! isset( $edi_settings['supplier_bacs_account'] ) ) {
				$bacs_account_options = woi_pdf_get_bacs_account_options();
		...
		WOI_PDF()->main->maybe_reinstall_fonts( true );
	}
	...
	protected function downgrade( $installed_version ) {
		...
		// Maybe reinstall fonts
		WOI_PDF()->main->maybe_reinstall_fonts();
	}
