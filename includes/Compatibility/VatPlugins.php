...
if ( ! class_exists( '\\WOI\\PDF\\Compatibility\\VatPlugins' ) ) :

/**
 * Detects third-party VAT number plugins active at checkout, so the plugin's
 * own checkout field can step aside and EDI/PDF documents can read the VAT
 * number from the right field.
 */
class VatPlugins {
	...
	public function detect(): array {
		...
		$defaults = apply_filters( 'woi_pdf_vat_plugin_checkout_selectors_defaults', array(
			'active'                => false,
			'key'                   => '',
			'classic_form_selector' => '#woi_pdf_checkout_field_field input',                             // our General Checkout field
			'block_form_selector'   => '.wc-block-components-address-form__wpo-ips-checkout-field input', // our General Checkout field
			'match'                 => null,
		) );

		$plugins = apply_filters(
			'woi_pdf_vat_plugin_detectors',
			array(
		...
		foreach ( $plugins as $key => $plugin ) {
			...
			return $this->detected = apply_filters( 'woi_pdf_vat_plugin_checkout_selectors', $out, $out['key'], $match );
		}
