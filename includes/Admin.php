...
class Admin {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_woi_pdf_delete_document', array( $this, 'ajax_crud_document' ) );
		add_action( 'wp_ajax_woi_pdf_regenerate_document', array( $this, 'ajax_crud_document' ) );
		add_action( 'wp_ajax_woi_pdf_save_document', array( $this, 'ajax_crud_document' ) );
		add_action( 'wp_ajax_woi_pdf_preview_formatted_number', array( $this, 'ajax_preview_formatted_number' ) );
		add_action( 'wp_ajax_woi_pdf_edi_save_order_customer_peppol_identifiers', array( $this, 'ajax_edi_save_order_customer_peppol_identifiers' ) );

		// document actions
		add_action( 'woi_pdf_document_actions', array( $this, 'add_regenerate_document_button' ) );
	...
	public function review_plugin_notice() {
		if ( $this->is_order_page() === false && !( isset( $_GET['page'] ) && $_GET['page'] == 'woi_pdf_options_page' ) ) {
			return;
		}

		if ( get_option( 'woi_pdf_review_notice_dismissed' ) !== false ) {
			return;
		} else {
			if ( isset( $_REQUEST['woi_pdf_dismiss_review'] ) && isset( $_REQUEST['_wpdismissnonce'] ) ) {
				// validate nonce
				if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['_wpdismissnonce'] ) ), 'dismiss_review_nonce' ) ) {
					woi_pdf_log_error( 'You do not have sufficient permissions to perform this action: woi_pdf_dismiss_review' );
					return;
				} else {
					update_option( 'woi_pdf_review_notice_dismissed', true );
					return;
				}
			}
			...
			if ( $invoice_count > 100 ) {
				...
				$notice_shown_on = get_option( 'woi_pdf_review_notice_shown', array() );
				$today           = gmdate( 'Y-m-d' );
				if ( ! in_array( $today, $notice_shown_on ) ) {
					$notice_shown_on[] = $today;
					update_option( 'woi_pdf_review_notice_shown', $notice_shown_on );
				}
				...
				?>
				<div class="notice notice-info is-dismissible woi-pdf-review-notice">
					<h3>
						<?php
							printf(
								/* translators: rounded count */
								esc_html__( 'Wow, you have created more than %d invoices with our plugin!', 'woocommerce-orders-invoice-pdf' ),
								esc_html( $rounded_count )
							);
						?>
					</h3>
					<p><?php esc_html_e( 'It would mean a lot to us if you would quickly give our plugin a 5-star rating. Help us spread the word and boost our motivation!', 'woocommerce-orders-invoice-pdf' ); ?></p>
					<ul>
						<li><a href="https://wordpress.org/support/plugin/woocommerce-pdf-invoices-packing-slips/reviews/?rate=5#new-post" class="button"><?php esc_html_e( 'Yes you deserve it!', 'woocommerce-orders-invoice-pdf' ); ?></span></a></li>
						<li><a href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'woi_pdf_dismiss_review', true ), 'dismiss_review_nonce', '_wpdismissnonce' ) ); ?>" class="woi-pdf-dismiss"><?php esc_html_e( 'Hide this message', 'woocommerce-orders-invoice-pdf' ); ?> / <?php esc_html_e( 'Already did!', 'woocommerce-orders-invoice-pdf' ); ?></a></li>
						<li><a href="mailto:support@wpovernight.com?Subject=Here%20is%20how%20I%20think%20you%20can%20do%20better"><?php esc_html_e( 'Actually, I have a complaint...', 'woocommerce-orders-invoice-pdf' ); ?></a></li>
					</ul>
				</div>
				<script type="text/javascript">
				jQuery( function( $ ) {
					$( '.woi-pdf-review-notice' ).on( 'click', '.notice-dismiss', function( event ) {
						event.preventDefault();
						window.location.href = $( '.woi-pdf-dismiss' ).attr('href');
					});
				});
				</script>
				<!-- Hide extensions ad if this is shown -->
				<style>.woi-pdf-extensions-ad { display: none; }</style>
				<?php
			}
		}
	}
	...
	public function install_wizard_notice() {
		...
		if ( get_option( 'woi_pdf_install_notice_dismissed' ) !== false ) {
			return;
		} else {
			if ( isset( $_REQUEST['woi_pdf_dismiss_install'] ) && isset( $_REQUEST['_wpdismissnonce'] ) ) {
				// validate nonce
				if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['_wpdismissnonce'] ) ), 'dismiss_install_nonce' ) ) {
					woi_pdf_log_error( 'You do not have sufficient permissions to perform this action: woi_pdf_dismiss_install' );
					return;
				} else {
					update_option( 'woi_pdf_install_notice_dismissed', true );
					return;
				}
			}
			...
			if ( get_transient( 'woi_pdf_new_install' ) !== false ) {
				...
				<script type="text/javascript">
				jQuery( function( $ ) {
					$( '.woi-pdf-install-notice' ).on( 'click', '.notice-dismiss', function( event ) {
						event.preventDefault();
						window.location.href = $( '.woi-pdf-dismiss-wizard' ).attr('href');
					});
				});
				</script>
				<?php
	...
	public function update_pdf_counter( $document_type, $document ) {
		if ( in_array( $document_type, array('invoice','packing-slip') ) ) {
			$pdf_count = (int) get_option( 'woi_pdf_count_'.$document_type, 0 );
			update_option( 'woi_pdf_count_'.$document_type, $pdf_count + 1 );
		}
	}
	...
	public function add_listing_actions( $order ) {
		...
		foreach ( $documents as $document ) {
			...
			$icon           = ! empty( $document->icon ) ? $document->icon : WOI_PDF()->plugin_url() . '/assets/images/generic_document.svg';
			$document       = woi_pdf_get_document( $document_type, $order ); // reload document with order
					...
					switch ( $output_format ) {
						...
						case 'pdf':
							if ( $document->is_enabled( $output_format ) ) {
								$document_url     = WOI_PDF()->endpoint->get_document_link( $order, $document_type );
								...
								$listing_actions[ $document_type ] = array(
									...
									'class'         => apply_filters( 'woi_pdf_action_button_class', implode( ' ', $class ), $document ),
									'output_format' => $output_format,
								);
							}
							break;
						case 'xml':
							if ( $document->is_enabled( $output_format ) && woi_pdf_edi_is_available() ) {
								$document_url    = WOI_PDF()->endpoint->get_document_link( $order, $document_type, array( 'output' => $output_format ) );
								$document_title  = is_callable( array( $document, 'get_title' ) ) ? $document->get_title() : $document_title;
								$document_exists = is_callable( array( $document, 'exists' ) ) ? $document->exists() : false;
								$class           = array( $document_type, $output_format );
								...
								$listing_actions[ $document_type . "_{$output_format}" ] = array(
									...
									'printed'       => false,
									'edi'           => true,
									'class'         => apply_filters( 'woi_pdf_edi_action_button_class', implode( ' ', $class ), $document ),
									'output_format' => $output_format,
								);
							}
							break;
		...
		$listing_actions = apply_filters( 'woi_pdf_listing_actions', $listing_actions, $order );
		...
		foreach ( $listing_actions as $action => $data ) {
			...
			if ( isset( $data['output_format'] ) && ( 'xml' !== $data['output_format'] || $data['exists'] ) ) {
				printf(
					'<a href="%1$s" class="button tips woi_pdf %2$s" target="_blank" alt="%3$s" data-tip="%3$s" style="background-image:url(%4$s);">%5$s%6$s</a>',
					esc_url( $data['url'] ),
					esc_attr( $data['class'] ),
					esc_attr( $data['alt'] ),
					esc_attr( $data['img'] ),
	...
	public function add_invoice_columns( $columns ) {
		...
		if ( WOI_PDF()->order_util->custom_orders_table_usage_is_enabled() && 'woocommerce_page_wc-orders' !== $current_screen->id ) {
			return $columns;
		}

		// get invoice settings
		$invoice          = woi_pdf_get_document( 'invoice', null );
		$invoice_settings = $invoice->get_settings();
		$invoice_columns  = array(
			'invoice_number_column' => __( 'Invoice Number', 'woocommerce-orders-invoice-pdf' ),
			'invoice_date_column'   => __( 'Invoice Date', 'woocommerce-orders-invoice-pdf' ),
		);
	...
	public function invoice_columns_data( $column, $post_or_order_object ) {
		...
		switch ( $column ) {
			case 'invoice_number_column':
				...
				do_action( 'woi_pdf_invoice_number_column_end', $order );
				break;
			case 'invoice_date_column':
				$invoice_date = ! empty( $invoice ) && ! empty( $invoice->get_date() ) ? $invoice->get_date()->date_i18n( woi_pdf_date_format( $invoice, 'invoice_date_column' ) ) : '';
				echo esc_html( $invoice_date );
				do_action( 'woi_pdf_invoice_date_column_end', $order );
				break;
			default:
				return;
		}
	...
	public function invoice_columns_enabled() {
		$is_enabled       = false;
		$invoice_settings = get_option( 'woi_pdf_documents_settings_invoice', array() );
		$invoice_columns  = [
			...
			switch ( $query_vars['orderby'] ) {
				case 'invoice_number_column':
					$query_vars = array_merge( $query_vars, array(
						'meta_key' => '_woi_pdf_invoice_number',
						'orderby'  => apply_filters( 'woi_pdf_invoice_number_column_orderby', 'meta_value' ),
					) );
					break;
				case 'invoice_date_column':
					$query_vars = array_merge( $query_vars, array(
						'meta_key' => '_woi_pdf_invoice_date',
						'orderby'  => apply_filters( 'woi_pdf_invoice_date_column_orderby', 'meta_value' ),
					) );
					break;
				default:
					return $query_vars;
	...
	public function add_meta_boxes( $wc_screen_id, $wc_order ) {
		if ( WOI_PDF()->order_util->custom_orders_table_usage_is_enabled() ) {
		...
		add_meta_box(
			'woi_pdf_send_emails',
			__( 'Send order email', 'woocommerce-orders-invoice-pdf' ),
		...
		if ( ! empty( \WOI_PDF()->documents->get_documents( 'enabled', 'pdf' ) ) ) {
			// create PDF buttons
			add_meta_box(
				'woi_pdf-box',
				__( 'Create PDF', 'woocommerce-orders-invoice-pdf' ),
			...
			// Invoice number & date
			add_meta_box(
				'woi_pdf-data-input-box',
				__( 'PDF document data', 'woocommerce-orders-invoice-pdf' ),
		...
		// create EDI buttons
		if ( woi_pdf_edi_is_available() && ! empty( \WOI_PDF()->documents->get_documents( 'enabled', 'xml' ) ) ) {
			add_meta_box(
				'wpo_ips-edi-box',
				__( 'E-Documents', 'woocommerce-orders-invoice-pdf' ),
	...
	public function send_order_email_meta_box( $post_or_order_object ) {
		$order = ( $post_or_order_object instanceof \WP_Post ) ? wc_get_order( $post_or_order_object->ID ) : $post_or_order_object;
		?>
		<ul class="woi_pdf_send_emails order_actions submitbox">
			<li class="wide" id="actions" style="padding-left:0; padding-right:0; border:0;">
				<select name="woi_pdf_send_emails">
					<option value=""><?php esc_html_e( 'Choose an email to send&hellip;', 'woocommerce-orders-invoice-pdf' ); ?></option>
					<?php
					$mailer           = WC()->mailer();
					$order_emails     = array( 'new_order', 'cancelled_order', 'customer_processing_order', 'customer_completed_order', 'customer_invoice' );
					$available_emails = apply_filters_deprecated( 'woocommerce_resend_order_emails_available', array( $order_emails ), '3.5.7', 'woi_pdf_resend_order_emails_available' );
					$available_emails = apply_filters( 'woi_pdf_resend_order_emails_available', $available_emails, $order->get_id() );
			...
			<li class="wide" style="border:0; padding-left:0; padding-right:0; padding-bottom:0; float:left;">
				<input type="submit" class="button save_order button-primary" name="save" value="<?php esc_attr_e( 'Save order & send email', 'woocommerce-orders-invoice-pdf' ); ?>" />
				<?php
				$url = esc_url( wp_nonce_url( add_query_arg( 'woi_pdf_action', 'resend_email' ), 'generate_woi_pdf' ) );
				?>
			</li>
		</ul>
		<?php
	...
	public function pdf_actions_meta_box( $post_or_order_object ) {
		...
		foreach ( $documents as $document ) {
			$document_title = $document->get_title();
			$document       = woi_pdf_get_document( $document->get_type(), $order );

			if ( $document ) {
				$document_url          = WOI_PDF()->endpoint->get_document_link( $order, $document->get_type() );
				$document_title        = is_callable( array( $document, 'get_title' ) ) ? $document->get_title() : $document_title;
				$document_exists       = is_callable( array( $document, 'exists' ) ) ? $document->exists() : false;
				$document_printed      = $document_exists && is_callable( array( $document, 'printed' ) ) ? $document->printed() : false;
				...
				$document_settings     = get_option( 'woi_pdf_documents_settings_'.$document->get_type() ); // $document-settings might be not updated with the last settings
				$unmark_printed_url    = ! empty( $document_printed_data ) && isset( $document_settings['unmark_printed'] ) ? WOI_PDF()->endpoint->get_document_printed_link( 'unmark', $order, $document->get_type() ) : false;
				$manually_mark_printed = WOI_PDF()->main->document_can_be_manually_marked_printed( $document );
				$mark_printed_url      = $manually_mark_printed ? WOI_PDF()->endpoint->get_document_printed_link( 'mark', $order, $document->get_type() ) : false;
		...
		$meta_box_actions = apply_filters( 'woi_pdf_meta_box_actions', $meta_box_actions, $order->get_id() );

		?>
		<ul class="woi_pdf-actions">
			<?php
			foreach ( $meta_box_actions as $document_type => $data ) {
				...
				$manually_mark_printed = isset( $data['manually_mark_printed'] ) && $data['manually_mark_printed'] && ! empty( $data['mark_printed_url'] ) ? '<p class="printed-data">&#x21b3; <a href="' . $data['mark_printed_url'] . '">' . __( 'Mark printed', 'woocommerce-orders-invoice-pdf' ) . '</a></p>' : '';
				...
				$unmark_printed        = isset( $data['unmark_printed_url'] ) && $data['unmark_printed_url'] ? '<a class="unmark_printed" href="' . $data['unmark_printed_url'].'">' . __( 'Unmark', 'woocommerce-orders-invoice-pdf' ).'</a>' : '';
	...
	public function edi_actions_meta_box( object $post_or_order_object ): void {
		...
		$meta_box_actions = array();
		$documents        = WOI_PDF()->documents->get_documents( 'enabled', 'xml' );
		...
		$meta_box_actions = apply_filters( 'woi_pdf_edi_meta_box_actions', $meta_box_actions, $order->get_id() );

		if ( empty( $meta_box_actions ) ) {
			echo '<div class="notice notice-warning inline"><p>' . esc_html__( 'E-Documents require that the PDF version of the same document type is generated first.', 'woocommerce-orders-invoice-pdf' ) . '</p></div>';
		}

		if (
			( $order->get_cart_tax() > 0 || $order->get_shipping_tax() > 0 ) &&
			empty( woi_pdf_edi_get_tax_settings() )
		) {
			echo '<div class="notice notice-warning inline"><p>' .
				sprintf(
					/* translators: settings link */
					esc_html__( 'Tax Classification for E-Documents is not set. Please configure it in %s.', 'woocommerce-orders-invoice-pdf' ),
					'<a href="' . esc_url( admin_url( 'admin.php?page=woi_pdf_options_page&tab=edi&section=taxes' ) ) . '" target="_blank">' . esc_html__( 'the settings', 'woocommerce-orders-invoice-pdf' ) . '</a>'
				) .
			'</p></div>';
		}
		...
		<div class="edi-order-actions">
			<table class="widefat">
				<thead>
					<tr>
						<td>XML</td>
						<td><?php esc_html_e( 'Actions', 'woocommerce-orders-invoice-pdf' ); ?></td>
					</tr>
				</thead>
				<tbody>
					<?php
						foreach ( $meta_box_actions as $document_type => $data ) {
							...
							if ( ! empty( $network ) ) {
								...
								if ( 'sent' === $status ) {
									$label = sprintf(
										...
										esc_html__( '%s sent to Network', 'woocommerce-orders-invoice-pdf' ),
										esc_html( $alt )
									);

									$network_buttons = \woi_pdf_edi_generate_action_button_html(
										$dispatch_url,
									...
									if ( empty( $status ) ) {
										...
										if ( $action_id > 0 && ! empty( $scheduled_url ) ) {
											$send_label = sprintf(
												/* translators: document title */
												esc_html__( '%s dispatch already scheduled', 'woocommerce-orders-invoice-pdf' ),
												esc_html( $alt )
											);

											$send_button = \woi_pdf_edi_generate_action_button_html(
												$scheduled_url,
										...
										// Normal "Send to Network" button.
											...
											$send_label = sprintf(
												/* translators: document title */
												esc_html__( 'Send %s to Network', 'woocommerce-orders-invoice-pdf' ),
												esc_html( $alt )
											);
									...
									// Update + Resend
										...
										$resend_label = sprintf(
											/* translators: document title */
											esc_html__( 'Resend %s to Network', 'woocommerce-orders-invoice-pdf' ),
											esc_html( $alt )
										);

										$resend_button = \woi_pdf_edi_generate_action_button_html(
											$dispatch_url,
										...
										$update_label = sprintf(
											/* translators: document title */
											esc_html__( 'Update %s', 'woocommerce-orders-invoice-pdf' ),
											esc_html( $alt )
										);

										$update_button = \woi_pdf_edi_generate_action_button_html(
											$update_status_url,
							...
							printf(
								...
								sprintf(
									/* translators: document title */
									esc_html__( 'Download %s', 'woocommerce-orders-invoice-pdf' ),
									esc_html( $alt )
								),
	...
	public function ajax_edi_save_order_customer_peppol_identifiers(): void {
		// Nonce check.
		if ( ! check_ajax_referer( 'generate_woi_pdf', 'security', false ) ) {
			wp_send_json_error(
				array(
					'message' => __( 'Invalid security token.', 'woocommerce-orders-invoice-pdf' ),
				)
			);
		}
		...
		if ( ! current_user_can( 'edit_shop_orders' ) ) {
			wp_send_json_error(
				array(
					'message' => __( 'You do not have permission to perform this action.', 'woocommerce-orders-invoice-pdf' ),
				),
				403
			);
		}
		...
		if ( ! $order_id || empty( $values ) ) {
			wp_send_json_error(
				array(
					'message' => __( 'Invalid order ID or values.', 'woocommerce-orders-invoice-pdf' ),
				),
				400
			);
		}
		...
		if ( ! $order ) {
			wp_send_json_error(
				array(
					'message' => __( 'Order not found.', 'woocommerce-orders-invoice-pdf' ),
				),
				404
			);
		}
		...
		if ( ! current_user_can( 'edit_post', $order->get_id() ) ) {
			wp_send_json_error(
				array(
					'message' => __( 'You do not have permission to edit this order.', 'woocommerce-orders-invoice-pdf' ),
				),
				403
			);
		}
		...
		woi_pdf_edi_peppol_save_customer_identifiers( $customer_id, $values );
		woi_pdf_edi_maybe_save_order_peppol_data( $order, $values );

		wp_send_json_success(
			array(
				'message' => __( 'Peppol identifiers saved successfully.', 'woocommerce-orders-invoice-pdf' ),
			)
		);
	}
	...
	public function data_input_box_content( $post_or_order_object ) {
		...
		do_action( 'woi_pdf_meta_box_start', $order, $this );
		...
		if ( $invoice ) {
			...
			$data = array(
				...
				'date' => array(
					'label' => __( 'Invoice date:', 'woocommerce-orders-invoice-pdf' ),
				),
				'display_date' =>  array(
					'label' => __( 'Invoice display date:', 'woocommerce-orders-invoice-pdf' ),
				),
				'creation_trigger' =>  array(
					'label' => __( 'Invoice created via:', 'woocommerce-orders-invoice-pdf' ),
				),
				'notes' => array(
					'label' => __( 'Notes (displayed in the invoice):', 'woocommerce-orders-invoice-pdf' ),
				),
		...
		do_action( 'woi_pdf_meta_box_end', $order, $this );
	}
	...
	public function get_current_values_for_document_data( OrderDocument $document, array $data ): array {
		$current     = array();
		$name_prefix = "_woi_pdf_{$document->slug}_";
		...
		if ( ! isset( $current['number'] ) ) {
			...
			if (
				! empty( \WOI_PDF()->settings->debug_settings['default_manual_document_number'] ) &&
				'next_document_number' === \WOI_PDF()->settings->debug_settings['default_manual_document_number']
			) {
			...
			$current['number']['formatted'] = array(
				'value' => woi_pdf_format_document_number(
		...
		if ( ! empty( $data['creation_trigger'] ) ) {
			$document_triggers = WOI_PDF()->main->get_document_triggers();
		...
		return apply_filters( 'woi_pdf_current_values_for_document', $data, $document );
	}
	...
	public function output_number_date_edit_fields( $document, $data ): void {
		...
		$data = apply_filters( 'woi_pdf_document_data_meta_box_fields', $data, $document );
		...
		$document_data_editing_enabled = \WOI_PDF()->settings->user_can_manage_settings() &&
			( ! empty( \WOI_PDF()->settings->debug_settings['enable_document_data_editing'] ) || ! in_array( $document->get_type(), array( 'invoice', 'credit-note' ) ) );
		?>
		<div class="woi-pdf-data-fields" data-document="<?php echo esc_attr( $document->get_type() ); ?>" data-order_id="<?php echo esc_attr( $document->order->get_id() ); ?>">
			<section class="woi-pdf-data-fields-section number-date">
				<!-- Title -->
				<h4>
					...
					<?php if ( $document->exists() && ( isset( $data['number'] ) || isset( $data['date'] ) ) && $this->user_can_manage_document( $document->get_type() ) ) : ?>
						<span class="woi-pdf-edit-date-number dashicons dashicons-edit"></span>
						<span class="woi-pdf-delete-document dashicons dashicons-trash" data-action="delete" data-nonce="<?php echo esc_attr( wp_create_nonce( 'woi_pdf_delete_document' ) ); ?>"></span>
						<?php do_action( 'woi_pdf_document_actions', $document ); ?>
					<?php endif; ?>
				</h4>
				...
				<div class="read-only">
					<?php if ( $document->exists() ) : ?>
						...
						<?php if ( isset( $data['display_date'] ) || isset( $data['creation_trigger'] ) ) : ?>
							<div>
								<a href="#" class="view-more"><?php esc_html_e( 'View more details', 'woocommerce-orders-invoice-pdf' ); ?></a>
								<a href="#" class="hide-details" style="display:none;"><?php esc_html_e( 'Hide details', 'woocommerce-orders-invoice-pdf' ); ?></a>
							</div>
						<?php endif; ?>
						<?php do_action( 'woi_pdf_meta_box_after_document_data', $document, $document->order ); ?>
					<?php else : ?>
						<?php if ( $this->user_can_manage_document( $document->get_type() ) ) : ?>
							<?php if ( $document_data_editing_enabled ) : ?>
								<span class="woi-pdf-set-date-number button">
									<?php
										printf(
											/* translators: document title */
											esc_html__( 'Set %s number & date', 'woocommerce-orders-invoice-pdf' ),
							...
							<?php endif; ?>
						<?php else : ?>
							<p><?php echo esc_html__( 'You do not have sufficient permissions to edit this document.', 'woocommerce-orders-invoice-pdf' ); ?></p>
						<?php endif; ?>
					<?php endif; ?>
				</div>
						...
						<?php if ( ! empty( $data['number'] ) ) : ?>
							<div class="data-fields-grid">
								<div class="data-fields-row">
									<div class="field-group">
										<label for="<?php echo esc_attr( $data['number']['prefix']['name'] ); ?>">
											<?php esc_html_e( 'Number prefix', 'woocommerce-orders-invoice-pdf' ); ?>
											<?php
												$tip_text = sprintf(
													'%s %s',
													__( 'If set, this value will be used as number prefix.' , 'woocommerce-orders-invoice-pdf' ),
													sprintf(
									...
									<div class="field-group">
										<label for="<?php echo esc_attr( $data['number']['suffix']['name'] ); ?>">
											<?php esc_html_e( 'Number suffix', 'woocommerce-orders-invoice-pdf' ); ?>
											<?php
												$tip_text = sprintf(
													'%s %s',
													__( 'If set, this value will be used as number suffix.' , 'woocommerce-orders-invoice-pdf' ),
													sprintf(
														/* translators: 1. document slug, 2-3 placeholders */
														__( 'You can use the %1$s year and/or month with the %2$s or %3$s placeholders respectively.', 'woocommerce-orders-invoice-pdf' ),
														esc_html( $document->get_title() ),
									...
									<div class="field-group">
										<label for="<?php echo esc_attr( $data['number']['padding']['name'] ); ?>">
											<?php esc_html_e( 'Number padding', 'woocommerce-orders-invoice-pdf' ); ?>
											<?php
												$tip_text = sprintf(
								...
								<div class="data-fields-row">
										...
										<label for="<?php echo esc_attr( $data['number']['plain']['name'] ); ?>">
											<?php
												printf(
													...
													esc_html__( '%s number', 'woocommerce-orders-invoice-pdf' ),
													esc_html( $document->get_title() )
												);
											?>
										</label>
									...
									<div class="field-group">
										<label><?php esc_html_e( 'Formatted number', 'woocommerce-orders-invoice-pdf' ); ?></label>
									...
									<div class="row-note">
										<?php echo wp_kses_post( sprintf(
											/* translators: %1$s: open anchor tag, %2$s: close anchor tag */
											__( 'Manually changing the document\'s plain number also requires updating the next document number in the %1$sdocument settings%2$s.', 'woocommerce-orders-invoice-pdf' ),
											'<a href="' . esc_url( admin_url( 'admin.php?page=woi_pdf_options_page&tab=documents&section=' . $document->get_type() ) ) . '#next_' . $document->slug . '_number" target="_blank">',
											'</a>'
										) ); ?>
										<?php esc_html_e( 'Please note that changing the document number may create gaps in the numbering sequence.', 'woocommerce-orders-invoice-pdf' ); ?>
									</div>
								</div>
							</div>
						<?php endif; ?>
						<?php if ( isset( $data['date'] ) ) : ?>
							<div class="data-fields-grid">
								<div class="data-fields-row">
									<div class="field-group">
										<label for="<?php echo esc_attr( $data['date']['name'] ); ?>[date]">
											<?php
												printf(
													/* translators: %s document title */
													esc_html__( '%s date', 'woocommerce-orders-invoice-pdf' ),
													esc_html( $document->get_title() )
												);
											?>
										</label>
									...
									<div class="field-group">
										<label for="<?php echo esc_attr( $data['date']['name'] ); ?>[hour]"><?php esc_html_e( 'Hour', 'woocommerce-orders-invoice-pdf' ); ?></label>
										<input type="number" class="hour" placeholder="<?php esc_attr_e( 'h', 'woocommerce-orders-invoice-pdf' ); ?>" name="<?php echo esc_attr( $data['date']['name'] ); ?>[hour]" id="<?php echo esc_attr( $data['date']['name'] ); ?>[hour]" min="0" max="23" size="2" value="<?php echo esc_attr( $data['date']['hour'] ); ?>" pattern="([01]?[0-9]{1}|2[0-3]{1})" disabled="disabled">
									</div>
									<div class="field-group">
										<label for="<?php echo esc_attr( $data['date']['name'] ); ?>[minute]"><?php esc_html_e( 'Minute', 'woocommerce-orders-invoice-pdf' ); ?></label>
										<input type="number" class="minute" placeholder="<?php esc_attr_e( 'm', 'woocommerce-orders-invoice-pdf' ); ?>" name="<?php echo esc_attr( $data['date']['name'] ); ?>[minute]" id="<?php echo esc_attr( $data['date']['name'] ); ?>[minute]" min="0" max="59" size="2" value="<?php echo esc_attr( $data['date']['minute'] ); ?>" pattern="[0-5]{1}[0-9]{1}"  disabled="disabled">
									</div>
								</div>
							</div>
						<?php endif; ?>
				...
				<?php if ( array_key_exists( 'notes', $data ) ) : ?>
					<?php do_action( 'woi_pdf_meta_box_before_document_notes', $document, $document->order ); ?>
					<!-- Read only -->
					<div class="read-only">
						<span><strong><?php echo wp_kses_post( $data['notes']['label'] ); ?></strong></span>
						<?php if ( $this->user_can_manage_document( $document->get_type() ) ) : ?>
							<span class="woi-pdf-edit-document-notes dashicons dashicons-edit" data-edit="notes"></span>
						<?php endif; ?>
					...
					<div class="editable-notes">
						<div class="data-fields-grid">
							<div class="data-fields-row">
								<div class="field-group">
									<label for="<?php echo esc_attr( $data['notes']['name'] ); ?>"><?php esc_html_e( 'Notes', 'woocommerce-orders-invoice-pdf' ); ?></label>
								...
								<div class="field-group placeholder"></div> <!-- Empty cell -->
								<div class="field-group placeholder"></div> <!-- Empty cell -->
								<div class="row-note"><?php esc_html_e( 'Displayed in the document!', 'woocommerce-orders-invoice-pdf' ); ?></div>
							</div>
						</div>
					</div>
					<?php do_action( 'woi_pdf_meta_box_after_document_notes', $document, $document->order ); ?>
				<?php endif; ?>
			</section>

			<?php do_action( 'woi_pdf_meta_box_before_document_buttons', $document, $data ); ?>

			<!-- Save/Cancel buttons -->
			<section class="woi-pdf-data-fields-section woi-pdf-document-buttons">
				<div>
					<a class="button button-primary woi-pdf-save-document" data-nonce="<?php echo esc_attr( wp_create_nonce( 'woi_pdf_save_document' ) ); ?>" data-action="save"><?php esc_html_e( 'Save changes', 'woocommerce-orders-invoice-pdf' ); ?></a>
					<a class="button woi-pdf-cancel"><?php esc_html_e( 'Cancel', 'woocommerce-orders-invoice-pdf' ); ?></a>
				</div>
			</section>
			<!-- / Save/Cancel buttons -->
		</div>
	...
	public function add_regenerate_document_button( $document ) {
		...
		if ( $document->use_historical_settings() == true || isset( $document_settings['archive_pdf'] ) ) {
			printf( '<span class="woi-pdf-regenerate-document dashicons dashicons-update-alt" data-nonce="%s" data-action="regenerate"></span>', esc_attr( wp_create_nonce( 'woi_pdf_regenerate_document' ) ) );
		}
	}
	...
	public function bulk_actions( $actions ) {
		foreach ( woi_pdf_get_bulk_actions() as $action => $title ) {
	...
	public function save_invoice_number_date( $order_id, $order ) {
		...
		if ( 'shop_order' === $order_type ) {
			...
			if ( $invoice ) {
				...
				if ( $is_new ) {
					WOI_PDF()->main->log_document_creation_to_order_notes( $invoice, 'document_data' );
					WOI_PDF()->main->mark_document_printed( $invoice, 'document_data' );
				}
			}

			// allow other documents to hook here and save their form data
			do_action( 'woi_pdf_on_save_invoice_order_data', $form_data, $order, $this );
		}
	}
	...
	public function disable_storing_document_settings() {
		add_filter( 'woi_pdf_document_store_settings', array( $this, 'return_false' ), 9999 );
	}

	public function restore_storing_document_settings() {
		remove_filter( 'woi_pdf_document_store_settings', array( $this, 'return_false' ), 9999 );
	}
	...
	public function send_emails( $post_or_order_object_id, $post_or_order_object ) {
		...
		if ( ! empty( $_POST['woi_pdf_send_emails'] ) ) {
			$action = sanitize_text_field( wp_unslash( $_POST['woi_pdf_send_emails'] ) );
				...
				if ( ! empty( $mails ) ) {
					foreach ( $mails as $mail ) {
						if ( $mail->id == $email_to_send ) {
							...
							$order->add_order_note( sprintf( esc_html__( '%s email notification manually sent.', 'woocommerce-orders-invoice-pdf' ), $mail->title ), false, true );
						}
					}
				}
	...
	public function search_fields ( $custom_fields ) {
		$custom_fields[] = '_woi_pdf_invoice_number';
		$custom_fields[] = '_woi_pdf_formatted_invoice_number';
	...
	public function user_can_manage_document( $document_type ) {
		return apply_filters( 'woi_pdf_current_user_is_allowed', ( current_user_can( 'manage_woocommerce_orders' ) || current_user_can( 'edit_shop_orders' ) ), $document_type );
	}
	...
	public function ajax_crud_document() {
		if ( check_ajax_referer( 'woi_pdf_regenerate_document', 'security', false ) === false && check_ajax_referer( 'woi_pdf_save_document', 'security', false ) === false && check_ajax_referer( 'woi_pdf_delete_document', 'security', false ) === false ) {
			wp_send_json_error( array(
				'message' => esc_html__( 'Nonce expired!', 'woocommerce-orders-invoice-pdf' ),
			) );
		}

		$request = stripslashes_deep( $_POST );

		if ( ! isset( $request['action'] ) ||  ! in_array( $request['action'], array( 'woi_pdf_regenerate_document', 'woi_pdf_save_document', 'woi_pdf_delete_document' ) ) ) {
			wp_send_json_error( array(
				'message' => esc_html__( 'Bad action!', 'woocommerce-orders-invoice-pdf' ),
			) );
		}

		if ( empty( $request['order_id'] ) || empty( $request['document_type'] ) || empty( $request['action_type'] ) ) {
			wp_send_json_error( array(
				'message' => esc_html__( 'Incomplete request!', 'woocommerce-orders-invoice-pdf' ),
			) );
		}

		if ( ! $this->user_can_manage_document( $request['document_type'] ) ) {
			wp_send_json_error( array(
				'message' => esc_html__( 'No permissions!', 'woocommerce-orders-invoice-pdf' ),
			) );
		}
		...
		$notice_messages = array(
			'saved'       => array(
				'success' => __( 'Document data saved!', 'woocommerce-orders-invoice-pdf' ),
				'error'   => __( 'An error occurred while saving the document data!', 'woocommerce-orders-invoice-pdf' ),
			),
			'regenerated' => array(
				'success' => __( 'Document regenerated!', 'woocommerce-orders-invoice-pdf' ),
				'error'   => __( 'An error occurred while regenerating the document!', 'woocommerce-orders-invoice-pdf' ),
			),
			'deleted' => array(
				'success' => __( 'Document deleted!', 'woocommerce-orders-invoice-pdf' ),
				'error'   => __( 'An error occurred while deleting the document!', 'woocommerce-orders-invoice-pdf' ),
			),
		);

		try {
			$document = woi_pdf_get_document( $document_type, wc_get_order( $order_id ) );

			if ( ! empty( $document ) ) {

				// perform legacy date fields replacements check
				if ( isset( $form_data["_woi_pdf_{$document->slug}_date"] ) && ! is_array( $form_data["_woi_pdf_{$document->slug}_date"] ) ) {
				...
				if ( 'regenerate' === $action_type && $document->exists() ) {
					...
					WOI_PDF()->main->log_document_creation_trigger_to_order_meta( $document, 'document_data', true, $request );
					$response = array(
				...
				// on save
					...
					if ( $is_new ) {
						WOI_PDF()->main->log_document_creation_to_order_notes( $document, 'document_data' );
						WOI_PDF()->main->log_document_creation_trigger_to_order_meta( $document, 'document_data', false, $request );
						WOI_PDF()->main->mark_document_printed( $document, 'document_data' );
					}
					$response      = array(
						'message' => $notice_messages[$notice]['success'],
					);
				...
				// document not exist
					...
					$message_complement = __( 'Document does not exist.', 'woocommerce-orders-invoice-pdf' );
					wp_send_json_error( array(
				...
				$message_complement = __( 'Document is empty.', 'woocommerce-orders-invoice-pdf' );
				wp_send_json_error( array(
					'message' => wp_kses_post( $notice_messages[$notice]['error'] . ' ' . $message_complement ),
				) );
			}
	...
	public function legacy_date_fields_replacements( $form_data, $document_slug ) {
		$legacy_date   = sanitize_text_field( $form_data["_woi_pdf_{$document_slug}_date"] );
		$legacy_hour   = sanitize_text_field( $form_data["_woi_pdf_{$document_slug}_date_hour"] );
		$legacy_minute = sanitize_text_field( $form_data["_woi_pdf_{$document_slug}_date_minute"] );
		unset( $form_data["_woi_pdf_{$document_slug}_date_hour"] );
		unset( $form_data["_woi_pdf_{$document_slug}_date_minute"] );

		$form_data["_woi_pdf_{$document_slug}_date"] = array(
			'date'   => $legacy_date,
			'hour'   => $legacy_hour,
	...
	public function debug_enabled_warning( $wp_admin_bar ) {
		if ( isset(WOI_PDF()->settings->debug_settings['enable_debug']) && current_user_can( 'administrator' ) ) {
			$status_settings_url = 'admin.php?page=woi_pdf_options_page&tab=debug';
			$title = __( 'DEBUG output enabled', 'woocommerce-orders-invoice-pdf' );
			$args = array(
	...
	public function ajax_preview_formatted_number(): void {
		if ( ! check_ajax_referer( 'generate_woi_pdf', 'security', false ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid security token.', 'woocommerce-orders-invoice-pdf' ) ) );
		}
		...
		if ( empty( $order_id ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid order ID.', 'woocommerce-orders-invoice-pdf' ) ) );
		}

		$order = wc_get_order( $order_id );

		if ( ! $order ) {
			wp_send_json_error( array( 'message' => __( 'Invalid order.', 'woocommerce-orders-invoice-pdf' ) ) );
		}

		if ( empty( $document_type ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid document type.', 'woocommerce-orders-invoice-pdf' ) ) );
		}

		$document = woi_pdf_get_document( $document_type, $order );

		if ( ! $document ) {
			wp_send_json_error( array( 'message' => __( 'Invalid document.', 'woocommerce-orders-invoice-pdf' ) ) );
		}

		$formatted = woi_pdf_format_document_number( $plain, $prefix, $suffix, $padding, $document, $order );
	...
	public function process_order_document_form_data( array $form_data, $document ): array {
		if ( ! $document instanceof OrderDocument ) {
			...
			woi_pdf_log_error( $message, 'critical' );
		...
		if (
			check_ajax_referer( 'woi_pdf_regenerate_document', 'security', false ) === false &&
			check_ajax_referer( 'woi_pdf_save_document', 'security', false ) === false &&
			check_ajax_referer( 'woi_pdf_delete_document', 'security', false ) === false &&
		...
		$key_prefix                    = "_woi_pdf_{$document->slug}_";
		$document_data_editing_enabled = \WOI_PDF()->settings->user_can_manage_settings() &&
			( ! empty( \WOI_PDF()->settings->debug_settings['enable_document_data_editing'] ) || ! in_array( $document->get_type(), array( 'invoice', 'credit-note' ) ) );
		...
		return apply_filters( 'woi_pdf_order_document_form_data', $data, $form_data, $document );
	}

	public function add_invoice_number_to_order_report( $response ) {
		$order = wc_get_order( $response->data['order_id'] );
		if ( ! empty( $order ) ) {
			$response->data['invoice_number'] = $order->get_meta( '_woi_pdf_invoice_number' );
		}
	...
	public function add_invoice_number_header_to_order_export( $export_columns ) {
		$export_columns['invoice_number'] = __( 'Invoice Number', 'woocommerce-orders-invoice-pdf' );
	...
	public function sort_orders_by_numeric_invoice_number( $query ): void {
		if ( ! is_admin() || ! $query->is_main_query() || 'shop_order' !== $query->get( 'post_type' ) || '_woi_pdf_invoice_number' !== $query->get( 'meta_key' ) ) {
			return;
		}
	...
	private function get_order_meta_box_document_xml_action( string $document_type, \WC_Abstract_Order $order ): array {
		...
		$document_url     = WOI_PDF()->endpoint->get_document_link( $order, $document_type, array( 'output' => 'xml' ) );
		$document_title   = is_callable( array( $document, 'get_title' ) ) ? $document->get_title() : $document_title;
		...
		$xml_title        = sprintf(
			...
			$document_title,
			$number_formatted,
			$is_refund_order ? __( 'RFND', 'woocommerce-orders-invoice-pdf' ) : __( 'ORD', 'woocommerce-orders-invoice-pdf' ),
			$order->get_id()
		);
		...
		return array(
			...
			'alt'    => sprintf(
				/* translators: document title */
				__( 'XML %s', 'woocommerce-orders-invoice-pdf' ),
				$document_title
			),
			'title'  => $xml_title,
			'exists' => true,
	...
	private function get_order_meta_box_peppol_identifiers( \WC_Order $order ): void {
		if ( ! woi_pdf_edi_peppol_is_available() ) {
			return;
		}

		$identifiers_data   = woi_pdf_edi_get_order_customer_identifiers_data( $order );
		...
		?>
			<div class="edi-customer-identifiers">
				<table class="widefat">
					<thead>
						<tr>
							<td><?php esc_html_e( 'Identifier', 'woocommerce-orders-invoice-pdf' ); ?></td>
							<td class="collapse">
								<a href="#"><?php esc_html_e( 'Show', 'woocommerce-orders-invoice-pdf' ); ?></a>
							</td>
						</tr>
					</thead>
							...
							foreach ( $identifiers_data as $key => $identifier ) {
								...
								$display  = $value ?: sprintf(
									...
									$required
										...
										: 'optional',
									$required
										? esc_html__( 'Missing', 'woocommerce-orders-invoice-pdf' )
										: esc_html__( 'Optional', 'woocommerce-orders-invoice-pdf' )
								);
								?>
								<tr>
								<?php if ( 'full' === woi_pdf_edi_peppol_identifier_input_mode() ) : ?>
					...
					<?php if ( isset( $identifiers_data['vat_number'] ) ) : ?>
						...
						<tfoot>
							...
							<?php endif; ?>
								<td>
									<?php echo wp_kses_post( $display ); ?>
									<?php if ( 'vat_number' === $key && ! empty( $value ) && ! woi_pdf_edi_vat_number_has_country_prefix( $value ) ) : ?>
										<br><small class="notice-warning" style="color:#996800;"><?php esc_html_e( 'VAT number is missing the country prefix', 'woocommerce-orders-invoice-pdf' ); ?></small>
									<?php endif; ?>
								</td>
							</tr>
						</tfoot>
			...
			<?php if ( ! empty( $peppol_identifiers ) ) : ?>
				<div class="edi-customer-identifiers peppol">
					<table class="widefat">
						<thead>
							<tr>
								<td><?php esc_html_e( 'Peppol', 'woocommerce-orders-invoice-pdf' ); ?></td>
								<td class="editable">
									<a href="#"><?php esc_html_e( 'Edit', 'woocommerce-orders-invoice-pdf' ); ?></a>
								</td>
							</tr>
						</thead>
						<tbody>
						...
						<tfoot>
							<tr><td colspan="2" class="scheme-required"><?php esc_html_e( 'The identifier must be in "scheme:value" format (for example 0088:123456789).', 'woocommerce-orders-invoice-pdf' ); ?></td></tr>
							<tr>
								<td colspan="2">
									<a href="#" class="button button-primary" data-order_id="<?php echo absint( $order->get_id() ); ?>"><?php esc_html_e( 'Save', 'woocommerce-orders-invoice-pdf' ); ?></a>
									<a href="#" class="button cancel"><?php esc_html_e( 'Cancel', 'woocommerce-orders-invoice-pdf' ); ?></a>
								</td>
							</tr>
						</tfoot>
					</table>
	...
	private function is_invoice_number_numeric() {
		$invoice_settings = WOI_PDF()->settings->get_document_settings( 'invoice' );
		...
		return apply_filters( 'woi_pdf_invoice_number_is_numeric', $is_numeric );
	}
				...
				<?php
					echo wp_kses_post(
						sprintf(
							...
							sprintf(
								/* translators: %s document title */
								esc_html__( 'Editing of %s number and date is currently disabled.', 'woocommerce-orders-invoice-pdf' ),
								esc_html( $document->get_title() )
							),
							sprintf(
								/* translators: %1$s: open anchor tag, %2$s: close anchor tag, %3$s: setting name */
								esc_html__( 'If you need to enable this feature, you can do so in the %1$sAdvanced Settings%2$s section under %3$s.', 'woocommerce-orders-invoice-pdf' ),
								'<a href="' . esc_url( admin_url( 'admin.php?page=woi_pdf_options_page&tab=debug#enable_document_data_editing' ) ) . '" target="_blank">',
								'</a>',
								'<strong>' . esc_html__( 'Enable document data editing', 'woocommerce-orders-invoice-pdf' ) . '</strong>'
							)
						)
					);
				?>
