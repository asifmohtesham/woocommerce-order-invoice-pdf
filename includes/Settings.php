...
use WOI\PDF\Documents\SequentialNumberStore;
use WOI\PDF\Settings\SettingsCallbacks;
use WOI\PDF\Settings\SettingsGeneral;
use WOI\PDF\Settings\SettingsDocuments;
...
if ( ! class_exists( '\\WOI\\PDF\\Settings' ) ) :
...
class Settings {
	...
	public function __construct() {
		...
		add_filter( 'plugin_action_links_'.WOI_PDF()->plugin_basename, array( $this, 'add_settings_link' ) );
		add_filter( 'plugin_row_meta', array( $this, 'add_support_links' ), 10, 2 );

		// settings capabilities
		add_filter( 'option_page_capability_woi_pdf_general_settings', array( $this, 'user_settings_capability' ) );
		...
		add_action( 'woi_pdf_init_documents', array( $this, 'update_documents_settings_categories' ), 999 );

		// Apply categories to general settings.
		add_filter( 'woi_pdf_settings_fields_general', array( $this, 'update_general_settings_categories' ), 999, 5 );

		// Apply categories to debug (Advanced) settings.
		add_filter( 'woi_pdf_settings_fields_debug', array( $this, 'update_debug_settings_categories' ), 999, 4 );
	...
	public function menu() {
		...
		$this->options_page_hook = add_submenu_page(
			$parent_slug,
			esc_html__( 'PDF Invoices', 'woocommerce-orders-invoice-pdf' ),
			esc_html__( 'PDF Invoices', 'woocommerce-orders-invoice-pdf' ),
	...
	public function add_settings_link( $links ) {
		$action_links = array(
			'settings' => '<a href="admin.php?page=woi_pdf_options_page">'. esc_html__( 'Settings', 'woocommerce-orders-invoice-pdf' ) . '</a>',
		);
	...
	public function add_support_links( $links, $file ) {
		if ( $file == WOI_PDF()->plugin_basename ) {
			$row_meta = array(
				'docs'    => '<a href="https://docs.wpovernight.com/topic/woocommerce-pdf-invoices-packing-slips/" target="_blank" title="' . esc_html__( 'Documentation', 'woocommerce-orders-invoice-pdf' ) . '">' . esc_html__( 'Documentation', 'woocommerce-orders-invoice-pdf' ) . '</a>',
				'support' => '<a href="https://wordpress.org/support/plugin/woocommerce-pdf-invoices-packing-slips" target="_blank" title="' . esc_html__( 'Support Forum', 'woocommerce-orders-invoice-pdf' ) . '">' . esc_html__( 'Support Forum', 'woocommerce-orders-invoice-pdf' ) . '</a>',
			);
	...
	public function user_settings_capability() {
		...
		$default_capability = apply_filters( 'woi_pdf_settings_default_user_capability', $manage_woocommerce );
		$default_capability = ( empty( $default_capability ) || ! is_string( $default_capability ) ) ? $manage_woocommerce : $default_capability;

		// Get the list of capabilities
		$capabilities = (array) apply_filters( 'woi_pdf_settings_user_role_capabilities', array( $default_capability ) );
	...
	public function settings_page() {
		...
		$settings_tabs = apply_filters( 'woi_pdf_settings_tabs', array(
			'general'   => array(
	...
	public function ajax_preview() {
		...
		try {
			...
			if ( ! empty( $_POST['order_id'] ) ) {
				...
				$default_order_id = wc_get_orders( apply_filters( 'woi_pdf_preview_default_order_id_query_args', array(
					'limit'  => 1,
					'return' => 'ids',
					'type'   => 'shop_order',
				), $document_type ) );
				$order_id = apply_filters( 'woi_pdf_preview_default_order_id', ! empty( $default_order_id ) ? reset( $default_order_id ) : false );
			}

			// get PDF data for preview
			if ( $order_id ) {
				$order = apply_filters( 'woi_pdf_preview_order_object', wc_get_order( $order_id ), $order_id, $document_type );

				if ( empty( $order ) ) {
					wp_send_json_error( array( 'error' => esc_html__( 'Order not found!', 'woocommerce-orders-invoice-pdf' ) ) );
				}
				if ( ! in_array( $order->get_type(), array( 'shop_order', 'shop_order_refund' ) ) ) {
					wp_send_json_error( array( 'error' => esc_html__( 'Object found is not an order!', 'woocommerce-orders-invoice-pdf' ) ) );
				}
				...
				if ( ! empty( $_POST['data'] ) ) {
					...
					foreach ( $form_data as $option_key => $form_settings ) {
						if ( ! empty( $option_key ) && false === apply_filters( 'woi_pdf_preview_filter_option', 0 === strpos( $option_key, 'woi_pdf' ), $option_key ) ) {
				...
				if ( $document ) {
					if ( ! $document->exists() ) {
						...
						$number_store_name   = apply_filters( 'woi_pdf_document_sequential_number_store', "{$document->slug}_number", $document );
					...
					wp_send_json_error(
						array(
							'error' => sprintf(
								...
								esc_html__( 'The PDF preview for order #%1$d is not available. This can happen if some settings prevent the document from being generated. Please review your configuration or check the %2$sdocumentation%3$s for more details.', 'woocommerce-orders-invoice-pdf' ),
								$order_id,
				...
				wp_send_json_error( array( 'error' => esc_html__( 'No WooCommerce orders found! Please consider adding your first order to see this preview.', 'woocommerce-orders-invoice-pdf' ) ) );
			}

		} catch ( \Throwable $th ) {
			woi_pdf_log_error( 'Error trying to generate document: ' . $th->getTraceAsString(), 'critical' );

			wp_send_json_error(
				array(
					'error' => sprintf(
						/* translators: error message */
						esc_html__( 'Error trying to generate document: %s', 'woocommerce-orders-invoice-pdf' ),
						$th->getMessage()
					)
				)
			);
	...
	public function get_document_settings( $document_type, $output_format = 'pdf' ) {
		if ( ! empty( $document_type ) ) {
			$option_name = ( 'pdf' === $output_format || 'xml' === $output_format ) // In 5.0.0 and later, E‑Documents settings are isolated from document settings, so PDF is the default.
				? "woi_pdf_documents_settings_{$document_type}"
				: "woi_pdf_documents_settings_{$document_type}_{$output_format}";
	...
	public function get_output_format( $document = null, $request = null ) {
		...
		return apply_filters( 'woi_pdf_output_format', $output_format, $document );
	}
		...
		foreach ( $installed_templates as $path => $template_id ) {
			...
			switch ( $group ) {
				...
				case 'premium_plugin':
					...
					break;
				case 'extension':
					$template_name = sprintf( '%s (%s) [%s]', $template_name, __( 'Extension', 'woocommerce-orders-invoice-pdf' ), $extension[1] );
					break;
				case 'theme':
				default:
					$template_name = sprintf( '%s (%s)', $template_name, __( 'Custom', 'woocommerce-orders-invoice-pdf' ) );
					break;
			}
	...
	public function get_template_path( string $template_path = '' ) {
		...
		if ( empty( $selected_template ) ) {
			return wp_normalize_path( WOI_PDF()->plugin_path() . '/templates/Simple' );
		}
	...
	public function get_installed_templates( $force_reload = false ) {
		...
		if ( ! did_action( 'woi_pdf_init_documents' ) && ( $cached_template_list = $this->get_template_list_cache() ) ) {
		...
		$installed_templates = apply_filters( 'woi_pdf_installed_templates', $installed_templates );
	...
	public function get_template_list_cache() {
		$template_list = get_option( 'woi_pdf_installed_template_paths', array() );
	...
	public function set_template_list_cache( $template_list ) {
		...
		update_option( 'woi_pdf_installed_template_paths', $template_list );
	}

	public function delete_template_list_cache() {
		delete_option( 'woi_pdf_installed_template_paths' );
	}
	...
	public function maybe_delete_flush_rewrite_rules_transient() {
		if ( get_transient( 'woi_pdf_flush_rewrite_rules' ) ) {
			flush_rewrite_rules();
			delete_transient( 'woi_pdf_flush_rewrite_rules' );
		}
	}
	...
	public function maybe_migrate_template_paths( $settings_section = null, $nonce = null ) {
		...
		if ( ! in_array( $selected_template, $installed_templates ) && substr_count( $selected_template, '/' ) > 1 ) {
			...
			if ( ! empty( $template_match ) ) {
				...
				update_option( 'woi_pdf_settings_general', $this->general_settings );
				/* translators: 1. path, 2. template ID */
				woi_pdf_log_error( sprintf( 'Template setting migrated from %1$s to %2$s', $path, $template_id ), 'info' );
			}
		}
	}
	...
	public function schedule_yearly_reset_numbers() {
		...
		if ( ! function_exists( '\\as_schedule_single_action' ) || ! function_exists( '\\as_get_scheduled_actions' ) ) {
			woi_pdf_log_error( 'Action Scheduler functions not available. Cannot schedule yearly document number reset.', 'critical' );
			return;
		}
		...
		$hook      = 'woi_pdf_schedule_yearly_reset_numbers';
		...
		if ( $scheduled_actions < 1 ) {
			...
			if ( function_exists( '\\as_unschedule_all_actions' ) ) {
				...
				woi_pdf_log_error( 'Action Scheduler functions not available. Cannot unschedule yearly document number reset.', 'critical' );
			}
	...
	public function maybe_schedule_yearly_reset_numbers() {
		...
		if ( ! $schedule ) {
			if ( function_exists( '\\as_unschedule_all_actions' ) ) {
				\as_unschedule_all_actions( 'woi_pdf_schedule_yearly_reset_numbers' );
			} else {
				woi_pdf_log_error( 'Action Scheduler functions not available. Cannot unschedule yearly document number reset.', 'critical' );
			}
		}
	...
	public function yearly_reset_action_is_scheduled() {
		...
		if ( ! function_exists( '\\as_get_scheduled_actions' ) ) {
			woi_pdf_log_error( 'Action Scheduler function not available. Cannot check if the yearly numbering reset is scheduled.', 'critical' );
			return $is_scheduled;
		}
		...
		if ( ! empty( $scheduled_actions ) ) {
			...
			if ( $total_actions === 1 ) {
				...
				$message = sprintf(
					...
					__( 'Only 1 scheduled action should exist for the yearly reset of the numbering system, but %s were found', 'woocommerce-orders-invoice-pdf' ),
					$total_actions
				);
				woi_pdf_log_error( $message );
			}
		}
	...
	public function get_media_upload_setting_html() {
		check_ajax_referer( 'woi_pdf_get_media_upload_setting_html', 'security' );
	...
	public function update_documents_settings_categories(): void {
		...
		foreach ( $documents as $document ) {
			foreach ( $document->output_formats as $output_format ) {
				add_filter(
					"woi_pdf_settings_fields_documents_{$document->get_type()}_{$output_format}",
	...
	public function apply_document_settings_categories( array $settings_fields ): array {
		...
		$document       = woi_pdf_get_document( $document_type, null );
	 ...
	 * @return array
	 */
	public function update_general_settings_categories( array $settings_fields, string $page, string $option_group, string $option_name, \WOI\PDF\Settings\SettingsGeneral $general_settings ): array {
	...
	public function apply_setting_categories( array $settings_fields, array $settings_categories ): array {
		...
		if ( ! empty( $unprocessed_settings_fields ) ) {
			...
			$modified_settings_fields[] = $this->create_section(
				$category_name,
				__( 'Additional settings', 'woocommerce-orders-invoice-pdf' )
			);
	...
	public function sync_shop_address_with_woo(): void {
		...
		if ( empty( $address_field ) ) {
			wp_send_json_error( array( 'message' => __( 'Address field is required.', 'woocommerce-orders-invoice-pdf' ) ) );
			return;
		}
		...
		if ( ! array_key_exists( $address_field, $address_map ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid address field.', 'woocommerce-orders-invoice-pdf' ) ) );
			return;
		}
		...
		if ( empty( $raw_value ) ) {
			wp_send_json_error( array( 'message' => __( 'The field is empty.', 'woocommerce-orders-invoice-pdf' ) ) );
			return;
		}
