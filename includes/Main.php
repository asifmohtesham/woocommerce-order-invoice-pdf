...
use WOI\PDF\Vendor\Dompdf\Exception as DompdfException;
...
class Main {
	...
	public function __construct() {
		add_action( 'wp_ajax_generate_woi_pdf', array( $this, 'generate_document_ajax' ) );
		add_action( 'wp_ajax_nopriv_generate_woi_pdf', array( $this, 'generate_document_ajax' ) );
		add_action( 'wp_ajax_woi_pdf_get_refund_order_ids', array( $this, 'get_refund_order_ids_ajax' ) );
		add_action( 'wp_ajax_nopriv_woi_pdf_get_refund_order_ids', array( $this, 'get_refund_order_ids_ajax' ) );

		// mark/unmark printed
		add_action( 'wp_ajax_printed_woi_pdf', array( $this, 'document_printed_ajax' ) );

		// email
		add_filter( 'woocommerce_email_attachments', array( $this, 'attach_document_to_email' ), 99, 4 );
		add_filter( 'woi_pdf_document_is_allowed', array( $this, 'disable_free' ), 10, 2 );
		...
		// test mode
		add_filter( 'woi_pdf_document_use_historical_settings', array( $this, 'test_mode_settings' ), 15, 2 );

		// page numbers & currency filters
		add_filter( 'woi_pdf_get_html', array( $this, 'format_page_number_placeholders' ), 10, 2 );
		add_action( 'woi_pdf_after_dompdf_render', array( $this, 'page_number_replacements' ), 9, 2 );
		add_filter( 'woi_pdf_pdf_filters', array( $this, 'pdf_currency_filters' ) );
		add_filter( 'woi_pdf_html_filters', array( $this, 'html_currency_filters' ) );
		...
		// remove private data
		if ( apply_filters( 'woi_pdf_remove_order_personal_data', true ) ) {
			...
			add_filter( 'woi_pdf_document_is_allowed', array( $this, 'disable_anonymized' ), 11, 2 );
		}
		// export private data
		...
		add_action( 'woi_pdf_save_document', array( $this, 'wc_webhook_trigger' ), 10, 2 );
	...
	public function attach_document_to_email( $attachments, $email_id, $order, $email = null ) {
		...
		$order = apply_filters( 'woi_pdf_email_order_object', $order, $email_id, $email );
		...
		add_filter( 'woi_pdf_disable_deprecation_notices', '__return_true' );
		...
		if ( apply_filters( 'woi_pdf_allow_reload_attachment_translations', isset( WOI_PDF()->settings->debug_settings['reload_attachment_translations'] ) ) ) {
			WOI_PDF()->translations();
			do_action( 'woi_pdf_reload_attachment_translations' );
		}
		...
		if ( $semaphore->lock() ) {
			...
			foreach ( $attach_to_document_types as $output_format => $document_types ) {
				foreach ( $document_types as $document_type ) {
					$email_order    = apply_filters( 'woi_pdf_email_attachment_order', $order, $email, $document_type );
					$email_order_id = $email_order->get_id();

					do_action( 'woi_pdf_before_attachment_creation', $email_order, $email_id, $document_type );

					try {
						// log document generation to order notes
						add_action( 'woi_pdf_init_document', function( $document ) {
						...
						$document = woi_pdf_get_document( $document_type, (array) $email_order_id, true );
						if ( ! $document ) { // something went wrong, continue trying with other documents
							woi_pdf_log_error( "Couldn't get the document object for email attachment. document type: {$document_type}, output format: {$output_format}, email order ID: #{$email_order_id}", 'critical' );
							continue;
						}

						$attachment = woi_pdf_get_document_file( $document, $output_format );

						if ( $attachment ) {
							$attachments[] = $attachment;
							if ( ! empty( WOI_PDF()->settings->debug_settings['log_to_order_notes'] ) ) {
								...
								$note = sprintf(
									...
									__( '%1$s %2$s successfully attached to the %3$s email.', 'woocommerce-orders-invoice-pdf' ),
							...
							woi_pdf_log_error( sprintf( 'PDF %1$s could not be attached to email (%2$s).', $document->get_title(), $email_id ), 'critical' );
							continue;
						}

						do_action( 'woi_pdf_email_attachment', $attachment, $document_type, $document, $output_format );

					} catch ( \Exception $e ) {
						woi_pdf_log_error( $e->getMessage(), 'critical', $e );
						continue;
					} catch ( DompdfException $e ) {
						woi_pdf_log_error( 'DOMPDF exception: '.$e->getMessage(), 'critical', $e );
						continue;
					} catch ( \Error $e ) {
						woi_pdf_log_error( $e->getMessage(), 'critical', $e );
						continue;
		...
		remove_filter( 'woi_pdf_disable_deprecation_notices', '__return_true' );
	...
	public function get_document_pdf_attachment( $document, $tmp_path ) {
		...
		$lock_file        = apply_filters( 'woi_pdf_lock_attachment_file', true );
		...
		try {
			...
			if ( WOI_PDF()->file_system->exists( $pdf_path ) && $reuse_attachment && $max_reuse_age > 0 ) {
				$filemtime = WOI_PDF()->file_system->mtime( $pdf_path );
			...
			$pdf_data  = WOI_PDF()->renderer->render( $document->get_html() );
			...
			// Write the file
			if ( $write_file ) {
				$file_written = WOI_PDF()->file_system->put_contents( $pdf_path, $pdf_data, FS_CHMOD_FILE );
			...
			woi_pdf_log_error( "Exception occurred: " . $e->getMessage(), 'critical' );
			return false;
		} finally {
	...
	public function get_document_xml_attachment( $document, $tmp_path ) {
		return woi_pdf_edi_write_file( $document, true );
	}
	...
	public function get_documents_for_email( $email_id, $order ) {
		...
		foreach ( $documents as $document ) {
			...
			if ( ! function_exists( 'WOI_PDF_Pro' ) && 'packing-slip' === $document->get_type() ) {
				continue;
			}
			...
			foreach ( $document->output_formats as $output_format ) {
				if ( $document->is_enabled( $output_format ) ) {
					if (
						'xml' === $output_format &&
						(
							! woi_pdf_edi_is_available()     ||
							! woi_pdf_edi_send_attachments()
						)
					) {
						continue;
					}
		...
		$attach_documents = apply_filters( 'woi_pdf_attach_documents', $attach_documents );
		...
		foreach ( $attach_documents as $output_format => $_documents ) {
			foreach ( $_documents as $document_type => $attach_to_email_ids ) {
				...
				$extra_condition = apply_filters( 'woi_pdf_custom_attachment_condition', true, $order, $email_id, $document_type, $output_format );
		...
		return apply_filters( 'woi_pdf_document_types_for_email', $document_types, $email_id, $order );
	}
	...
	public function generate_document_ajax(): void {
		$access_type  = WOI_PDF()->endpoint->get_document_link_access_type();
		$redirect_url = WOI_PDF()->endpoint->get_document_denied_frontend_redirect_url();
		...
		if ( empty( $access_key ) ) {
			$message = esc_attr__( 'You do not have sufficient permissions to access this page. Reason: empty access key', 'woocommerce-orders-invoice-pdf' );
			woi_pdf_safe_redirect_or_die( $redirect_url, $message );
		}

		// check if we have the action
		if ( empty( $action) ) {
			$message = esc_attr__( 'You do not have sufficient permissions to access this page. Reason: empty action', 'woocommerce-orders-invoice-pdf' );
			woi_pdf_safe_redirect_or_die( $redirect_url, $message );
		}

		// Check the nonce for logged in users
		if ( is_user_logged_in() && 'logged_in' === $access_type && ! $valid_nonce ) {
			$message = esc_attr__( 'You do not have sufficient permissions to access this page. Reason: invalid nonce', 'woocommerce-orders-invoice-pdf' );
			woi_pdf_safe_redirect_or_die( $redirect_url, $message );
		}
		...
		if ( empty( $request['order_ids'] ) ) {
			$message = esc_attr__( "You haven't selected any orders", 'woocommerce-orders-invoice-pdf' );
			woi_pdf_safe_redirect_or_die( null, $message );
		}

		if ( empty( $request['document_type'] ) ) {
			$message = esc_attr__( 'Some of the export parameters are missing.', 'woocommerce-orders-invoice-pdf' );
			woi_pdf_safe_redirect_or_die( null, $message );
		}
		...
		if ( count( $order_ids ) === 1 ) {
			...
			if ( $order && $order->get_status() == 'auto-draft' ) {
				$message = esc_attr__( 'You have to save the order before generating a PDF document for it.', 'woocommerce-orders-invoice-pdf' );
				woi_pdf_safe_redirect_or_die( null, $message );
			} elseif ( ! $order ) {
				$message = sprintf(
					/* translators: %s: Order ID */
					esc_attr__( 'Could not find the order #%s.', 'woocommerce-orders-invoice-pdf' ),
					$order_id
				);
				woi_pdf_safe_redirect_or_die( null, $message );
			}
		...
		$sort_order         = apply_filters( 'woi_pdf_bulk_document_sort_order', 'ASC' );
		...
		$full_permission = WOI_PDF()->admin->user_can_manage_document( $document_type );
		...
		$allowed = apply_filters( 'woi_pdf_check_privs', $allowed, $order_ids );

		if ( ! $allowed ) {
			$message = esc_attr__( 'You do not have sufficient permissions to access this page.', 'woocommerce-orders-invoice-pdf' );
			woi_pdf_safe_redirect_or_die( $redirect_url, $message );
		}
		...
		try {
			...
			foreach ( $order_ids as $check_order_id ) {
				$check_doc = woi_pdf_get_document( $document_type, $check_order_id );
				$pre_existing[ $check_order_id ] = $check_doc && is_callable( array( $check_doc, 'exists' ) ) && $check_doc->exists();
			}

			// get document
			$document = woi_pdf_get_document( $document_type, $order_ids, true );

			if ( $document ) {
				do_action( 'woi_pdf_document_created_manually', $document, $order_ids ); // note that $order_ids is filtered and may not be the same as the order IDs used for the document (which can be fetched from the document object itself with $document->order_ids)

				// Iterate explicitly so pre-existing documents are also processed; woi_pdf_init_document only fires for newly created docs.
				...
				foreach ( $document_order_ids as $individual_order_id ) {
					$individual_document = woi_pdf_get_document( $document_type, $individual_order_id, true );
				...
				$output_format = WOI_PDF()->settings->get_output_format( $document, $request );
				...
				switch ( $output_format ) {
					case 'xml':
						...
						break;
					case 'html':
						add_filter( 'woi_pdf_use_path', '__return_false' );
						$document->output_html();
						break;
					case 'pdf':
					default:
						if ( has_action( 'woi_pdf_created_manually' ) ) {
							do_action( 'woi_pdf_created_manually', $document->get_pdf(), $document->get_filename() );
						}
						$output_mode = WOI_PDF()->settings->get_output_mode( $document_type );
				...
				$message = sprintf(
					...
					esc_html__( "Document of type '%s' for the selected order(s) could not be generated", 'woocommerce-orders-invoice-pdf' ),
					$document_type
				);
				woi_pdf_safe_redirect_or_die( null, $message );
			}
			...
			woi_pdf_log_error( $message, 'critical', $e );
			woi_pdf_output_error( $message, 'critical', $e );
		} catch ( \Exception $e ) {
	...
	public function get_refund_order_ids_ajax(): void {
		...
		// - bulk generate (generate_woi_pdf)
		// - cloud export (pro_cloud_storage)

		$valid_nonce = check_ajax_referer( 'generate_woi_pdf', 'security', false )
			|| check_ajax_referer( 'pro_cloud_storage', 'security', false );

		if ( ! $valid_nonce ) {
			wp_send_json_error(
				array(
					'message' => __( 'Invalid request. Nonce check failed.', 'woocommerce-orders-invoice-pdf' ),
				)
			);
		}
		...
		if ( empty( $_POST['order_ids'] ) ) {
			wp_send_json_error(
				array(
					'message' => __( 'No orders were provided.', 'woocommerce-orders-invoice-pdf' ),
				)
			);
		}

		$order_ids  = array_map( 'absint', (array) $_POST['order_ids'] );
		$refund_ids = \woi_pdf_get_refund_ids( $order_ids );

		if ( empty( $refund_ids ) ) {
			wp_send_json_error(
				array(
					'message' => __( 'No refunds found for the selected orders.', 'woocommerce-orders-invoice-pdf' ),
				)
			);
		}
	...
	private function load_template_functions() {
		...
		if ( isset( $_POST['action'] ) && 'woi_pdf_preview' === sanitize_text_field( wp_unslash( $_POST['action'] ) ) && ! empty( $_POST['data'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
			// parse form data
			...
			$selected_id = sanitize_text_field( $form_data['woi_pdf_settings_general']['template_path'] ?? '' );

			// only allow template ids that exist in our installed templates list
			$installed_templates = array_keys( WOI_PDF()->settings->get_installed_templates_list() );
			if ( in_array( $selected_id, $installed_templates, true ) ) {
				$template_path = $selected_id;
			}
		}

		$file = trailingslashit( WOI_PDF()->settings->get_template_path( $template_path ) ) . 'template-functions.php';
		if ( WOI_PDF()->file_system->exists( $file ) ) {
			$loaded = @include_once( $file );
			if ( $loaded === false ) {
				woi_pdf_log_error( sprintf( 'Failed to load template functions: %s', $file ), 'critical' );
			}
		}
	}
	...
	public function get_tmp_path( $type = '' ) {
		...
		if ( ! WOI_PDF()->file_system->is_dir( $tmp_base ) || ! WOI_PDF()->file_system->is_writable( $tmp_base ) ) {
			$this->init_tmp();
		}
		...
		if ( ! WOI_PDF()->file_system->is_dir( $tmp_path ) ) {
			$dir = WOI_PDF()->file_system->mkdir( $tmp_path );

			if ( ! $dir ) {
				update_option( 'woi_pdf_no_dir_error', $tmp_path );
				woi_pdf_log_error( "Unable to create folder {$tmp_path}", 'critical' );
				return false;
			}
		} elseif( ! WOI_PDF()->file_system->is_writable( $tmp_path ) ) {
			update_option( 'woi_pdf_no_dir_error', $tmp_path );
			woi_pdf_log_error( "Temp folder {$tmp_path} not writable", 'critical' );
			return false;
		}

		return apply_filters( "woi_pdf_tmp_path_{$type}", $tmp_path );
	}
	...
	public function get_tmp_base ( $append_random_string = true ) {
		...
		// 'woi_pdf' folder and several subfolders are created
		//
		// wp_upload_dir() will:
		...
		//
		// May also be overridden by the woi_pdf_tmp_path filter

		$wp_upload_base = $this->get_wp_upload_base();
		if( $wp_upload_base ) {
			if( $append_random_string && $code = $this->get_random_string() ) {
				$tmp_base = $wp_upload_base . 'woi_pdf_'.$code.'/';
			} else {
				$tmp_base = $wp_upload_base . 'woi_pdf/';
			}
		} else {
			$tmp_base = false;
		}

		$tmp_base = apply_filters( 'woi_pdf_tmp_path', $tmp_base );
		if ($tmp_base !== false) {
	...
	public function tmp_subfolder_has_files( string $subfolder ): bool {
		if ( empty( $subfolder ) || ! in_array( $subfolder, $this->subfolders, true ) ) {
			woi_pdf_log_error( sprintf( 'The directory %s is not a default tmp subfolder from this plugin.', $subfolder ), 'critical' );
			return false;
		}

		$cache_key = "woi_pdf_subfolder_{$subfolder}_has_files";
		...
		try {
			...
			woi_pdf_log_error( 'Error reading directory: ' . $e->getMessage(), 'critical' );
			return false;
		}
	...
	public function maybe_reinstall_fonts( bool $force = false ): void {
		...
		if ( ! $has_font_files || $force ) {
			...
			if ( function_exists( 'glob' ) && $files = glob( $fonts_path.'/*.*' ) ) {
				...
				woi_pdf_log_error( "Couldn't clear fonts tmp subfolder before copy fonts.", 'critical' );
			}
			...
			// save to cache
			set_transient( 'woi_pdf_subfolder_fonts_has_files', 'yes' , DAY_IN_SECONDS );
		}
	}
	...
	public function generate_random_string() {
		...
		// create option
		update_option( 'woi_pdf_random_string', $code );
	}
	...
	public function get_random_string () {
		$code = get_option( 'woi_pdf_random_string', '' );
		if ( ! empty( $code ) ) {
	...
	public function init_tmp() {
		...
		if ( ! WOI_PDF()->file_system->is_dir( $tmp_base ) ) {
			$dir = WOI_PDF()->file_system->mkdir( $tmp_base );

			// don't continue if we don't have an upload dir
			if ( ! $dir ) {
				update_option( 'woi_pdf_no_dir_error', $tmp_base );
				woi_pdf_log_error( "Unable to create temp folder {$tmp_base}", 'critical' );
				return false;
			}
		} elseif( ! WOI_PDF()->file_system->is_writable( $tmp_base ) ) {
			update_option( 'woi_pdf_no_dir_error', $tmp_base );
			woi_pdf_log_error( "Temp folder {$tmp_base} not writable", 'critical' );
			return false;
		}
		...
		foreach ( $this->subfolders as $subfolder ) {
			...
			if ( ! WOI_PDF()->file_system->is_dir( $path ) ) {
				$dir = WOI_PDF()->file_system->mkdir( $path );

				// check if we have dir
				if ( ! $dir ) {
					update_option( 'woi_pdf_no_dir_error', $path );
					woi_pdf_log_error( "Unable to create folder {$path}", 'critical' );
					return false;
				}
			} elseif( ! WOI_PDF()->file_system->is_writable( $path ) ) {
				update_option( 'woi_pdf_no_dir_error', $path );
				woi_pdf_log_error( "Temp folder {$path} not writable", 'critical' );
				return false;
			}
			...
			WOI_PDF()->file_system->put_contents( $path . '.htaccess', 'deny from all', FS_CHMOD_FILE );
			WOI_PDF()->file_system->put_contents( $path . 'index.php', '', FS_CHMOD_FILE );
		}
	}

	public function no_dir_notice() {
		if( is_admin() && ( $path = get_option( 'woi_pdf_no_dir_error' ) ) ) {
			// if all folders exist and are writable delete the option
			if( $this->tmp_folders_exist_and_writable() ) {
				delete_option( 'woi_pdf_no_dir_error' );
			// if not, show notice
			} else {
				if ( $path ) {
					ob_start();
						...
						<p>
							<?php
								printf(
									...
									wp_kses_post( 'The %1$s directory %2$s couldn\'t be created or is not writable!', 'woocommerce-orders-invoice-pdf' ),
									'<strong>PDF Invoices & Packing Slips for WooCommerce</strong>',
									'<code>' . woi_pdf_escape_url_path_or_base64( $path ) . '</code>' // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
								);
							?>
						</p>
						<p><?php esc_html_e( 'Please check your directories write permissions or contact your hosting service provider.', 'woocommerce-orders-invoice-pdf' ); ?></p>
						<p><a href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'woi_pdf_hide_no_dir_notice', 'true' ), 'hide_no_dir_notice_nonce' ) ); ?>"><?php esc_html_e( 'Hide this message', 'woocommerce-orders-invoice-pdf' ); ?></a></p>
					</div>
					<?php
					echo wp_kses_post( ob_get_clean() );

					// save option to hide notice
					if ( isset( $_REQUEST['woi_pdf_hide_no_dir_notice'] ) && isset( $_REQUEST['_wpnonce'] ) ) {
						// validate nonce
						if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['_wpnonce'] ) ), 'hide_no_dir_notice_nonce' ) ) {
							woi_pdf_log_error( 'You do not have sufficient permissions to perform this action: woi_pdf_hide_no_dir_notice' );
							wp_safe_redirect( 'admin.php?page=woi_pdf_options_page' );
							exit;
						} else {
							delete_option( 'woi_pdf_no_dir_error' );
							wp_safe_redirect( 'admin.php?page=woi_pdf_options_page' );
	...
	public function copy_directory( $old_path, $new_path ) {
		...
		if ( ! WOI_PDF()->file_system->is_dir( $old_path ) ) {
			return;
		}

		if ( ! WOI_PDF()->file_system->is_dir( $new_path ) ) {
			$dir = WOI_PDF()->file_system->mkdir( $new_path );

			// check if we have dir
			if ( ! $dir ) {
				update_option( 'woi_pdf_no_dir_error', $new_path );
				woi_pdf_log_error( "Unable to create folder {$new_path}", 'critical' );
				return false;
			}
		} elseif ( ! WOI_PDF()->file_system->is_writable( $new_path ) ) {
			update_option( 'woi_pdf_no_dir_error', $new_path );
			woi_pdf_log_error( "Temp folder {$new_path} not writable", 'critical' );
			return false;
		}
		...
		try {
			...
			if ( $result ) {
				WOI_PDF()->file_system->delete( $old_path, true );
			}
		} catch ( \Error $e ) {
			woi_pdf_log_error( "Unable to copy directory contents: ".$e->getMessage(), 'critical', $e );
			return;
		}
	}
	...
	private function tmp_folders_exist_and_writable()
	{
		...
		foreach( $this->subfolders as $type ) {
			...
			if( ! WOI_PDF()->file_system->is_dir( $tmp_path ) || ! WOI_PDF()->file_system->is_writable( $tmp_base ) ) {
				return false;
			}
		}
	...
	public function copy_fonts( $path = '', $merge_with_local = true ) {
		...
		if ( $default_pdf_maker !== apply_filters( 'woi_pdf_pdf_maker', $default_pdf_maker ) ) {
			return;
		}
		...
		$dompdf_options = apply_filters( 'woi_pdf_dompdf_options', array(
		...
		$synchronizer = WOI_PDF()->font_synchronizer;
	...
	public function disable_anonymized( $allowed, $document ) {
		if ( ! empty( $document->order ) && ! empty( $anonymized = $document->order->get_meta( '_anonymized' ) ) ) {
			if ( apply_filters( 'woi_pdf_disallow_anonymized_order_document', wc_string_to_bool( $anonymized ), $this ) ) {
				$allowed = false;
			}
		}
		return $allowed;
	...
	public function test_mode_settings( $use_historical_settings, $document ) {
		if ( isset( WOI_PDF()->settings->general_settings['test_mode'] ) ) {
	...
	public function pdf_currency_filters( $filters ) {
		if ( isset( WOI_PDF()->settings->general_settings['currency_font'] ) ) {
			$filters[] = array( 'woocommerce_currency_symbol', array( $this, 'use_currency_font' ), 10001, 2 );
			// 'woi_pdf_custom_styles' is actually an action, but WP handles them with the same functions
			$filters[] = array( 'woi_pdf_custom_styles', array( $this, 'currency_symbol_font_styles' ) );
		}
		return $filters;
	}
	...
	public function html_currency_filters( $filters ) {
		...
		if ( isset( $_POST['action'] ) && 'woi_pdf_preview' === sanitize_text_field( wp_unslash( $_POST['action'] ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
			$filters = $this->pdf_currency_filters( $filters );
		}

		// Only apply these fixes if the default PDF maker is used
		if ( woi_pdf_pdf_maker_is_default() ) {
	...
	public function schedule_temporary_files_cleanup() {
		if ( ! isset( WOI_PDF()->settings->debug_settings['enable_cleanup'] ) ) {
			return;
		}

		$cleanup_age_days = isset( WOI_PDF()->settings->debug_settings['cleanup_days'] ) ? floatval( WOI_PDF()->settings->debug_settings['cleanup_days'] ) : 7.0;
	...
	public function temporary_files_cleanup( int $delete_timestamp = 0 ): array {
		...
		$paths_to_cleanup = apply_filters( 'woi_pdf_cleanup_tmp_paths', array(
			$this->get_tmp_path( 'attachments' ),
			$this->get_tmp_path( 'dompdf' ),
		) );
		$excluded_files   = apply_filters( 'woi_pdf_cleanup_excluded_files', array(
			'index.php',
			'.htaccess',
			'log.htm',
		) );
		apply_filters_deprecated( 'woi_pdf_cleanup_folders_level', array( 3 ), '3.9.1', '', 'This filter is no longer necessary.' );
		$files            = array();
		$success          = 0;
		$error            = 0;
		$output           = array();
		...
		foreach ( $paths_to_cleanup as $path ) {
			if ( WOI_PDF()->file_system->is_dir( $path ) ) {
				$listed_files = WOI_PDF()->file_system->dirlist( $path, true, true );
				...
				if ( $listed_files ) {
					foreach ( $listed_files as $fileinfo ) {
						...
						if ( ! in_array( $basename, $excluded_files ) && WOI_PDF()->file_system->exists( $file_path ) && ! WOI_PDF()->file_system->is_dir( $file_path ) ) {
							$files[] = $file_path;
						}
					}
				}
		...
		// No files to delete
		if ( empty( $files ) ) {
			$output['success'] = esc_html__( 'Nothing to delete!', 'woocommerce-orders-invoice-pdf' );
			return $output;
		}

		// Process and delete files
		foreach ( $files as $file ) {
			$file_timestamp = WOI_PDF()->file_system->mtime( $file );

			// Delete file if it's older than the specified timestamp
			if ( $file_timestamp < $delete_before ) {
				if ( WOI_PDF()->file_system->delete( $file ) ) {
					$success++;
				} else {
					$error++;
				}
	...
	public function remove_order_personal_data_meta( $meta_to_remove ) {
		$woi_pdf_private_meta = array(
			'_woi_pdf_invoice_number'         => 'numeric_id',
			'_woi_pdf_invoice_number_data'    => 'array',
			'_woi_pdf_invoice_date'           => 'timestamp',
			'_woi_pdf_invoice_date_formatted' => 'date',
		);
		return $meta_to_remove + $woi_pdf_private_meta;
	}
	...
	public function remove_order_personal_data( \WC_Abstract_Order $order ): void {
		...
		$number_stores = apply_filters( 'woi_pdf_privacy_number_stores', array( 'invoice_number' ) );

		foreach ( $number_stores as $store_name ) {
			$order_id   = $order->get_id();
			$table_name = apply_filters( 'woi_pdf_number_store_table_name', "{$wpdb->prefix}woi_pdf_{$store_name}", $store_name, 'auto_increment' ); // i.e. wp_woi_pdf_invoice_number

			$query = woi_pdf_prepare_identifier_query(
	...
	public function export_order_personal_data_meta( $meta_to_export ) {
		$private_address_meta = array(
			// _woi_pdf_invoice_number_data & _woi_pdf_invoice_date are duplicates of the below and therefore not included
			'_woi_pdf_invoice_number'         => esc_html__( 'Invoice Number', 'woocommerce-orders-invoice-pdf' ),
			'_woi_pdf_invoice_date_formatted' => esc_html__( 'Invoice Date', 'woocommerce-orders-invoice-pdf' ),
		);
	...
	public function log_document_creation_to_order_notes( object $document, string $trigger ) {
		if ( empty( $document ) || empty( $trigger ) || ! isset( WOI_PDF()->settings->debug_settings['log_to_order_notes'] ) ) {
			return;
		}
		...
		if ( array_key_exists( $trigger, $manual_triggers ) ) {
			...
			if ( ! empty( $user->user_login ) ) {
				$user_note = sprintf(
					' (%s: %s)',
					__( 'User', 'woocommerce-orders-invoice-pdf' ),
		...
		$note = sprintf(
			...
			__( 'PDF %1$s created via %2$s.', 'woocommerce-orders-invoice-pdf' ),
			$document->get_title(),
			$triggers[ $trigger ]
		);
	...
	public function log_document_deletion_to_order_notes( object $document ): void {
		if ( ! empty( WOI_PDF()->settings->debug_settings['log_to_order_notes'] ) ) {
			...
			$note = sprintf(
				...
				__( 'PDF %s deleted.', 'woocommerce-orders-invoice-pdf' ),
				$document->get_title()
			);
	...
	public function log_document_printed_to_order_notes( $document, $trigger ) {
		...
		if ( ! empty( $document ) && isset( WOI_PDF()->settings->debug_settings['log_to_order_notes'] ) && ! empty( $trigger ) && array_key_exists( $trigger, $triggers ) ) {
			/* translators: 1. document title, 2. creation trigger */
			$message = __( '%1$s document marked as printed via %2$s.', 'woocommerce-orders-invoice-pdf' );
	...
	public function log_unmark_document_printed_to_order_notes( $document ) {
		if ( ! empty( $document ) && isset( WOI_PDF()->settings->debug_settings['log_to_order_notes'] ) ) {
			/* translators: 1. document title, 2. creation trigger */
			$message = __( '%1$s document unmark printed.', 'woocommerce-orders-invoice-pdf' );
		...
		if ( ! empty( $order_ids ) ) {
			foreach ( $order_ids as $order_id ) {
				...
				if ( ! empty( $order ) ) {
					...
					$status = $order->get_meta( "_woi_pdf_{$document->slug}_creation_trigger" );

					if ( true == $force || empty( $status ) ) {
						$order->update_meta_data( "_woi_pdf_{$document->slug}_creation_trigger", $trigger );
	...
	public function get_document_triggers( string $trigger_type = 'all' ): array {
		$manual_triggers = apply_filters( 'woi_pdf_manual_document_triggers', array(
			'single'        => __( 'single order action', 'woocommerce-orders-invoice-pdf' ),
			'bulk'          => __( 'bulk order action', 'woocommerce-orders-invoice-pdf' ),
			'my_account'    => __( 'my account', 'woocommerce-orders-invoice-pdf' ),
			'document_data' => __( 'order document data (number and/or date set manually)', 'woocommerce-orders-invoice-pdf' ),
		) );

		$automatic_triggers = apply_filters( 'woi_pdf_automatic_document_triggers', array(
			'email_attachment' => __( 'email attachment', 'woocommerce-orders-invoice-pdf' ),
		) );
		...
		return apply_filters( 'woi_pdf_document_triggers', $triggers, $trigger_type );
	}
	...
	public function mark_document_printed( $document, $trigger ) {
		...
		if ( ! empty( $document ) && ! $this->is_document_printed( $document ) ) {
			if ( ! empty( $order = $document->order ) && ! empty( $trigger ) && in_array( $trigger, $triggers ) && apply_filters( 'woi_pdf_allow_mark_document_printed', true, $document, $trigger ) ) {
				if ( 'shop_order' === $order->get_type() ) {
					...
					$order->update_meta_data( "_woi_pdf_{$document->slug}_printed", $data );
					$order->save_meta_data();
	...
	public function unmark_document_printed( $document ) {
		if ( ! empty( $document ) && $this->is_document_printed( $document ) ) {
			if ( ! empty( $order = $document->order ) && apply_filters( 'woi_pdf_allow_unmark_document_printed', true, $document ) ) {
				$meta_key = "_woi_pdf_{$document->slug}_printed";
	...
	public function document_printed_ajax() {
		check_ajax_referer( 'printed_woi_pdf', 'security' );

		$data  = stripslashes_deep( $_REQUEST );
		$error = 0;

		if ( ! empty( $data['action'] ) && $data['action'] == "printed_woi_pdf" && ! empty( $data['event'] ) && ! empty( $data['document_type'] ) && ! empty( $data['order_id'] ) && ! empty( $data['trigger'] ) ) {
			$document        = woi_pdf_get_document( esc_attr( $data['document_type'] ), esc_attr( $data['order_id'] ) );
			$full_permission = WOI_PDF()->admin->user_can_manage_document( esc_attr( $data['document_type'] ) );
		...
		if ( $error > 0 ) {
			wp_die(
				sprintf(
					...
					esc_html__( 'Document of type %1$s for the selected order could not be marked as printed.', 'woocommerce-orders-invoice-pdf' ),
	...
	public function is_document_printed( $document ) {
		...
		if ( ! empty( $document ) && ! empty( $order = $document->order ) ) {
			if ( 'shop_order' === $order->get_type() && ! empty( $printed_data = $order->get_meta( "_woi_pdf_{$document->slug}_printed" ) ) ) {
				$is_printed = true;
			}
		}
	...
	public function document_can_be_manually_marked_printed( $document ) {
		...
		return apply_filters( 'woi_pdf_document_can_be_manually_marked_printed', $can_be_manually_marked_printed, $document );
	}
	...
	public function get_document_printed_data( $document ) {
		...
		return apply_filters( 'woi_pdf_document_printed_data', $data, $document );
	}
	...
	public function wc_webhook_topic_hooks( $topic_hooks, $wc_webhook ) {
		...
		foreach ($documents as $document) {
			$topic_hooks["order.{$document->type}-saved"] = array(
				"woi_pdf_webhook_order_{$document->slug}_saved",
			);
		}
		return $topic_hooks;
	}
	...
	public function wc_webhook_topics( $topics ) {
		...
		foreach ($documents as $document) {
			/* translators: document title */
			$topics["order.{$document->type}-saved"] = esc_html( sprintf( __( 'Order %s Saved', 'woocommerce-orders-invoice-pdf' ), $document->get_title() ) );
		}
		return $topics;
	}
	...
	public function wc_webhook_trigger( $document, $order ) {
		...
		do_action( "woi_pdf_webhook_order_{$document->slug}_saved", $order->get_id() );
	}
	 ...
	 * preventing the `woi_pdf_webhook_order_{$document->slug}_saved` action hook
	...
	private function reload_wpo_custom_webhooks() {
		if (
			! apply_filters( 'woi_pdf_reload_wpo_custom_webhooks', true ) ||
	...
	public function display_due_date_table_row( ?string $document_type = null, ?\WC_Abstract_Order $order = null ): void {
		...
		$current_template_path = explode( '/', WOI_PDF()->settings->get_template_path() );
		...
		if (
			function_exists( 'WOI_PDF_Templates' ) &&
			version_compare( WOI_PDF_Templates()->version, '2.21.9', '>' ) &&
		...
		$due_date = apply_filters_deprecated(
			'woi_pdf_due_date_display',
			array(
				date_i18n( woi_pdf_date_format( $this, 'due_date' ), $due_date_timestamp ),
				$due_date_timestamp,
				$document_type,
				$document
			),
			'3.9.0',
			'woi_pdf_document_due_date'
		);
		$due_date_title = is_callable( array( $document, 'get_due_date_title' ) ) ?
			$document->get_due_date_title() : __( 'Due Date:', 'woocommerce-orders-invoice-pdf' );
	...
	public function handle_document_link_in_emails(): void {
		...
		foreach ( $documents as $document ) {
			$document_settings = WOI_PDF()->settings->get_document_settings( $document->get_type(), 'pdf' );
		...
		$email_hooks = array_unique( apply_filters( 'woi_pdf_add_document_link_to_email_hooks', $email_hooks ) );
	...
	public function add_document_link_to_email( \WC_Abstract_Order $order, bool $sent_to_admin, bool $plain_text, $email ): void {
		...
		$is_full_access_type = 'full' === WOI_PDF()->endpoint->get_document_link_access_type();

		// Early exit if the requirements are not met
		if ( ! apply_filters( 'woi_pdf_add_document_link_to_email_requirements_met', $is_full_access_type, $order, $sent_to_admin, $plain_text, $email ) ) {
			return;
		}

		$allowed_document_types = apply_filters( 'woi_pdf_add_document_link_to_email_allowed_document_types', array( 'invoice' ), $order, $sent_to_admin, $plain_text, $email );
		$documents              = WOI_PDF()->documents->get_documents();
		...
		foreach ( $documents as $document ) {
			...
			if ( ! apply_filters( 'woi_pdf_add_document_link_to_email_is_allowed', $is_allowed, $order, $sent_to_admin, $plain_text, $email ) ) {
				continue;
			}
			...
			if (
				! $document->exists() &&
				apply_filters( 'woi_pdf_add_document_link_to_email_skip_missing_documents', false, $document, $order, $sent_to_admin, $plain_text, $email )
			) {
				continue;
			}
			...
			$link_text = sprintf(
				...
				__( 'View %s (PDF)', 'woocommerce-orders-invoice-pdf' ),
				wp_kses_post( $document->get_type() )
			);
			$link_url  = WOI_PDF()->endpoint->get_document_link( $order, $document->get_type(), array(), true );

			$document_link = sprintf(
				'<p><a id="%s" href="%s" target="_blank">%s</a></p>',
				esc_attr( 'woi_pdf_' . $document->get_type() . '_document_link' ),
				esc_url( $link_url ),
				esc_html( $link_text )
			);

			echo wp_kses_post( apply_filters( 'woi_pdf_add_document_download_link_to_email', $document_link, $document, $order, $sent_to_admin, $plain_text, $email ) );
		}
	}
	...
	public function apply_ink_saving_styles( string $css, OrderDocument $document ): string {
		$settings = WOI_PDF()->settings->general_settings ?? array();
		...
		$supported_templates = apply_filters(
			'woi_pdf_ink_saving_supported_templates',
