...
if ( ! class_exists( '\\WOI\\PDF\\FontSynchronizer' ) ) :
...
class FontSynchronizer {
	 ...
	 * @param bool   $merge_with_local if true, merge with existing local fonts
	 * @return void
	 */
	public function sync( string $destination, bool $merge_with_local = true ): void {
		...
		foreach( $plugin_fonts as $font_name => $filenames ) {
			$plugin_filenames = array_map( function( $file ) {
				return WOI_PDF()->plugin_path() . '/assets/fonts/' . $file;
			}, $filenames );
		...
		WOI_PDF()->file_system->put_contents( $destination . $this->font_cache_filename, $cacheData, FS_CHMOD_FILE );
	}
	...
	public function delete_font_files( array $filenames ): void {
		$plugin_folder = wp_normalize_path( WOI_PDF()->plugin_path() );
		...
		foreach ( $filenames as $filename ) {
			...
			foreach ( $extensions as $extension ) {
				...
				if ( WOI_PDF()->file_system->exists( $file ) ) {
					...
					woi_pdf_log_error( sprintf( "Could not delete font file (%s)", $file ), 'critical' );
					continue;
				}
			}
		}
	 ...
	 * @return array
	 */
	public function copy_font_files( array $filenames, string $destination ): array {
		...
		foreach ( $filenames as $variant => $filename ) {
			foreach ( $extensions as $extension ) {
				...
				if ( WOI_PDF()->file_system->is_readable( $file ) ) {
					$local_filename = $destination . basename( $file );
					
					if ( ! copy( $file, $local_filename ) ) {
						woi_pdf_log_error( sprintf( 'Failed to copy font file: %s to %s', $file, $local_filename ) );
					}
				} else {
					continue;
				}
	...
	public function get_local_fonts( string $path ): array {
		...
		if ( WOI_PDF()->file_system->is_readable( $cache_file ) ) {
			$json_data = WOI_PDF()->file_system->get_contents( $cache_file );
			$font_data = json_decode( $json_data, true );
		} elseif ( WOI_PDF()->file_system->is_readable( $legacy_cache_file ) ) {
		...
		if ( $font_data === false ) {
			woi_pdf_log_error( sprintf( "Could not read font cache file (%s)", $cache_file ), 'critical' );
		}
	 ...
	 * @return array
	 */
	public function get_plugin_fonts(): array {
		return array (
			'open sans'   => array (
	 ...
	 */
	public function normalize_font_paths( array $fonts ): array {
