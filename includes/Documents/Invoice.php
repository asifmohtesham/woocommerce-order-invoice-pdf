...
if ( ! class_exists( '\\WOI\\PDF\\Documents\\Invoice' ) ) :
...
class Invoice extends OrderDocumentMethods implements NumberedDocumentInterface, EmailAttachableInterface {
	...
	public function use_historical_settings(): bool {
		return apply_filters(
			'woi_pdf_document_use_historical_settings',
			woi_pdf_is_document_using_historical_settings( $this->get_type() ),
			$this
		);
	}
	...
	public function storing_settings_enabled(): bool {
		return apply_filters(
			'woi_pdf_document_store_settings',
			true,
			$this
		);
	}
	...
	public function init_number() {
		woi_pdf_deprecated_function( 'init_number', '3.8.0', 'initiate_number' );
	...
	public function init_settings() {
		do_action( "woi_pdf_before_{$this->type}_init_settings", $this );
		...
		foreach ( $this->output_formats as $output_format ) {
			...
			if ( 'pdf' === $output_format ) {
				$page            = $option_group = $option_name = "woi_pdf_documents_settings_{$this->get_type()}";
				$settings_fields = apply_filters( "woi_pdf_settings_fields_documents_{$this->get_type()}", $this->get_pdf_settings_fields( $option_name ), $page, $option_group, $option_name ); // legacy filter
			}

			// custom output format
			if ( empty( $page ) ) {
				$page = $option_group = $option_name = "woi_pdf_documents_settings_{$this->get_type()}_{$output_format}";
			}

			// allow plugins to alter settings fields
			$settings_fields = apply_filters( "woi_pdf_settings_fields_documents_{$this->type}_{$output_format}", $settings_fields, $page, $option_group, $option_name, $this );
		...
		do_action( "woi_pdf_after_{$this->type}_init_settings", $this );
	}
	...
	public function get_pdf_settings_fields( $option_name ) {
		...
		if ( 'full' === WOI_PDF()->endpoint->get_document_link_access_type() ) {
			$settings_fields[] = array(
				'type'     => 'setting',
		...
		return apply_filters( "woi_pdf_{$this->type}_pdf_settings_fields", $settings_fields, $option_name, $this );
	}
