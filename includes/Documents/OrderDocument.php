...
if ( ! class_exists( '\\WOI\\PDF\\Documents\\OrderDocument' ) ) :
...
abstract class OrderDocument implements DocumentInterface {
	...
	public function __construct( $order = 0 ) {
		...
		$this->output_formats = apply_filters_deprecated( "woi_pdf_{$this->slug}_output_formats", array( $this->output_formats, $this ), '3.8.7', 'woi_pdf_document_output_formats' );
	...
	public function init_settings_data(): void {
		...
		if (
			in_array( $preview_action, array( 'woi_pdf_preview', 'woi_pdf_preview' ), true ) &&
	...
	public function get_settings( $latest = false, $output_format = 'pdf' ) {
		...
		$common_settings   = WOI_PDF()->settings->get_common_document_settings();
		$document_settings = WOI_PDF()->settings->get_document_settings( $this->get_type(), $output_format );
	...
	public function initiate_number( bool $force_new_number = false ) {
		...
		$document_number = apply_filters( 'woi_pdf_initiate_number', $document_number, $this );
	...
	public function get_document_number( bool $generate = false ) {
		...
		if (
			...
			apply_filters( "woi_pdf_external_{$this->slug}_number_enabled", false, $this )
		) {
			...
			$document_number = apply_filters( "woi_pdf_external_{$this->slug}_number", $document_number, $this );
	...
	public function use_historical_settings() {
		return apply_filters( 'woi_pdf_document_use_historical_settings', false, $this );
	}

	public function storing_settings_enabled() {
		return apply_filters( 'woi_pdf_document_store_settings', false, $this );
	}

	public function get_non_historical_settings() {
		return apply_filters( 'woi_pdf_non_historical_settings', array(
			'enabled',
			'attach_to_email_ids',
	...
	public function is_enabled( $output_format = 'pdf' ): bool {
		...
		return apply_filters( 'woi_pdf_document_is_enabled', $is_enabled, $this->type, $output_format );
	}

	public function get_hook_prefix() {
		return 'woi_pdf_' . $this->slug . '_get_';
	}
	...
	public function save( $order = null ) {
		...
		do_action( 'woi_pdf_save_document', $this, $order );
	}
	...
	public function delete( $order = null ) {
		...
		$data_to_remove = apply_filters( 'woi_pdf_delete_document_data_keys', array(
			'settings',
			'date',
			'date_formatted',
			'number',
		...
		do_action( 'woi_pdf_delete_document', $this, $order );
	}
	...
	public function regenerate( $order = null, $data = null ) {
		...
		do_action( 'woi_pdf_regenerate_document', $this );
	}
	...
	public function is_allowed() {
		...
		return apply_filters( 'woi_pdf_document_is_allowed', $allowed, $this );
	}
	...
	public function printed() {
		return WOI_PDF()->main->is_document_printed( $this );
	}
	...
	*/
	...
	public function get_printed_data() {
		return WOI_PDF()->main->get_document_printed_data( $this );
	}
	...
	public function get_data( $key, $document_type = '', $order = null, $context = 'view' ) {
		...
		if ( $document_type !== $this->type ) {
			if ( ! isset( $this->linked_documents[ $document_type ] ) ) {
				...
				$this->linked_documents[ $document_type ] = woi_pdf_get_document( $document_type, null );
	...
	public function get_number( $document_type = '', $order = null, $context = 'view', $formatted = false ) {
		...
		return apply_filters( "woi_pdf_{$this->slug}_number", $number, $document_type, $order, $context, $formatted, $this );
	}
	...
	public function get_title_for( string $slug ): string {
		...
		$title = apply_filters( 'woi_pdf_title_for', $title, $slug, $this ); // used by Pro to translate strings

		return apply_filters( "woi_pdf_{$slug}_title", $title, $this );
	}
	...
	public function get_language_attributes(): string {
		$language_attributes = apply_filters( 'woi_pdf_document_language_attributes', get_language_attributes(), $this );
		$language_attributes = apply_filters_deprecated( 'woi_pdf_html_language_attributes', array( $language_attributes, $this->get_type(), $this ), '3.9.1', 'woi_pdf_document_language_attributes' );
	...
	public function get_body_class(): string {
		$body_class = apply_filters( 'woi_pdf_document_body_class', $this->get_type(), $this );
		return apply_filters_deprecated( 'woi_pdf_body_class', array( $body_class, $this ), '3.9.1', 'woi_pdf_document_body_class' );
	}
	...
	*/
	...
	public function get_number_settings() {
		...
		return apply_filters( 'woi_pdf_document_number_settings', $number_settings, $this );
	}
	...
	public function template_styles() {
		$css_file_path = apply_filters( 'woi_pdf_template_styles_file', $this->locate_template_file( 'style.css' ) );
		$css           = '';

		if ( WOI_PDF()->file_system->exists( $css_file_path ) ) {
			ob_start();
			include $css_file_path;
			$css = ob_get_clean();
		}

		$css = apply_filters( 'woi_pdf_template_styles', $css, $this );
	...
	public function template_custom_styles(): void {
		...
		do_action( 'woi_pdf_custom_styles', $this->get_type(), $this );

		$css = apply_filters( 'woi_pdf_template_custom_styles', ob_get_clean(), $this );
	...
	public function get_header_logo_id(): int {
		...
		$header_logo_id = apply_filters( 'woi_pdf_header_logo_id', $header_logo_id, $this );
	...
	public function get_settings_text( $settings_key, $default = false, $autop = true ) {
		...
		if ( in_array( $settings_key, array( 'shop_name', 'footer', 'extra_1', 'extra_2', 'extra_3' ) ) ) {
			$text = apply_filters_deprecated(
				"woi_pdf_{$settings_key}",
				array( $text, $this ),
				'4.5.0',
				"woi_pdf_{$settings_key}_settings_text"
			);
		} elseif ( 'shop_address' === $settings_key ) {
			$text = apply_filters_deprecated(
				'woi_pdf_shop_address_settings_text',
				array( $text, $this ),
				'4.6.0',
				'woi_pdf_get_shop_address'
			);
		}

		return apply_filters( "woi_pdf_{$settings_key}_settings_text", $text, $this );
	}
	...
	public function get_shop_address_country(): string {
		return woi_pdf_get_country_name_from_code( $this->get_shop_address_country_code() );
	}
	...
	public function get_shop_address(): string {
		...
		return apply_filters(
			'woi_pdf_get_shop_address',
			woi_pdf_format_address( $address ),
			$address,
			$this
		);
	}
	public function shop_address() {
		echo esc_html( apply_filters( 'woi_pdf_shop_address', $this->get_shop_address(), $this ) );
	}
	...
	public function get_footer() {
		ob_start();
		do_action( 'woi_pdf_before_footer', $this->get_type(), $this->order );
		echo $this->get_settings_text( 'footer' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		do_action( 'woi_pdf_after_footer', $this->get_type(), $this->order );
		return ob_get_clean();
	}
	...
	*/
	...
	public function get_pdf() {
		...
		$pdf_file = apply_filters( 'woi_pdf_load_pdf_file_path', null, $this );

		if ( $pdf_file ) {
			$pdf = WOI_PDF()->file_system->get_contents( $pdf_file );
		} else {
			$pdf = null;
		}

		$pdf = apply_filters( 'woi_pdf_pdf_data', $pdf, $this );
		...
		return apply_filters( 'woi_pdf_get_pdf', $pdf, $this );
	}
	...
	public function wrap_html_content( $content ) {
		$html = $this->render_template( $this->locate_template_file( "html-document-wrapper.php" ), array(
				'content' => apply_filters( 'woi_pdf_html_content', $content ),
			)
		);
		return $html;
	}
	...
	public function get_template_path() {
		return WOI_PDF()->settings->get_template_path();
	}
	...
	public function locate_template_file( $file ) {
		...
		$file_path          = "{$path}/{$file}";
		$fallback_file_path = WOI_PDF()->plugin_path() . '/templates/Simple/' . $file;

		if ( ! WOI_PDF()->file_system->exists( $file_path ) && WOI_PDF()->file_system->exists( $fallback_file_path ) ) {
			$file_path = $fallback_file_path;
		}

		$file_path = apply_filters( 'woi_pdf_template_file', $file_path, $this->type, $this->order );

		return $file_path;
	}

	public function render_template( $file, $args = array() ) {
		do_action( 'woi_pdf_process_template', $this->get_type(), $this );
	...
	*/
	...
	public function get_wc_emails() {
		...
		if ( empty( $request['page'] ) || ! in_array( $request['page'], array( 'woi-pdf-setup', 'woi_pdf_options_page' ) ) ) {
			return array();
		}
		...
		if (function_exists('WC')) {
			...
			if ( empty( $woocommerce ) ) { // bail if WooCommerce not active
				return apply_filters( 'woi_pdf_wc_emails', array() );
			}
		...
		return apply_filters( 'woi_pdf_wc_emails', $emails );
	}
	...
	public function get_sequential_number_store() {
		...
		if ( $reset_number_yearly ) {
			...
			if ( apply_filters( 'woi_pdf_enable_yearly_reset_debug', false ) ) {
			...
			if ( $number_store->is_new ) {
				$number_store->set_next( apply_filters( 'woi_pdf_reset_number_yearly_start', 1, $this ) );
			}
		// reset: off
		} else {
	...
	public function get_sequential_number_store_name( $date, $method, $reset_number_yearly ) {
		$store_base_name    = $this->order ? apply_filters( 'woi_pdf_document_sequential_number_store', "{$this->slug}_number", $this ) : "{$this->slug}_number";
		...
		return apply_filters( "woi_pdf_{$this->slug}_number_store_name", $number_store_name, $store_base_name, $date, $method, $this );
	}
	...
	public function maybe_retire_number_store( $date, $store_base_name, $method ) {
		...
		$retired_table_safe = woi_pdf_sanitize_identifier( $retired_table_name );
		...
		if ( $retired_exists ) {
			$drop_query = woi_pdf_prepare_identifier_query(
		...
		$default_table_safe = woi_pdf_sanitize_identifier( $default_table_name );
		...
		if ( $default_exists ) {
			$rename_query = woi_pdf_prepare_identifier_query(
		...
		$current_year_table_safe = woi_pdf_sanitize_identifier( $current_year_table_name );
	...
	public function get_number_store_year( string $table_name ): int {
		...
		if ( $table_exists ) {
			...
			$year_query = woi_pdf_prepare_identifier_query(
	...
	public function calculate_due_date( int $due_date_days ): int {
		$due_date_days = apply_filters_deprecated(
			'woi_pdf_due_date_days',
			array( $due_date_days, $this->get_type(), $this ),
			'3.8.7',
			'woi_pdf_document_due_date_days'
		);
		$due_date_days = apply_filters( 'woi_pdf_document_due_date_days', $due_date_days, $this );
		...
		$base_date              = apply_filters_deprecated(
			'woi_pdf_due_date_base_date',
			array( $document_creation_date, $this->get_type(), $this ),
			'3.8.7',
			'woi_pdf_document_due_date_base_date'
		);
		$base_date              = apply_filters( 'woi_pdf_document_due_date_base_date', $base_date, $this );
		...
		return apply_filters( 'woi_pdf_document_due_date', $due_date ?? 0, $this );
	}
