...
if ( ! class_exists( '\\WOI\\PDF\\Documents\\SequentialNumberStore' ) ) :
...
class SequentialNumberStore {
	...
	public function __construct( string $store_name, string $method = 'auto_increment' ) {
		...
		$this->table_name = apply_filters(
			'woi_pdf_number_store_table_name',
	...
	public function init(): void {
		...
		if ( ! $this->store_name_exists() ) {
			...
			if ( 'calculate' === $this->method ) {
				...
				$column_check_query = woi_pdf_prepare_identifier_query(
				...
				if ( empty( $column_exists ) ) {
					...
					$alter_query = woi_pdf_prepare_identifier_query(
...
$sql = "CREATE TABLE {$this->table_name} (
  ...
  KEY order_id (order_id)
	...
	public function increment( int $order_id = 0, ?string $date = null ): int {
		...
		do_action( 'woi_pdf_before_sequential_number_increment', $this, $order_id, $date );
	...
	public function set_next( int $number = 1 ): void {
		// Delete all rows
		$truncate_query = woi_pdf_prepare_identifier_query(
			"TRUNCATE TABLE %i",
