...
if ( ! class_exists( '\\WOI\\PDF\\Documents\\OrderDocumentMethods' ) ) :
...
abstract class OrderDocumentMethods extends OrderDocument {
	...
	public function ships_to_different_address() {
		...
		if ( $formatted_shipping_address = $order->get_formatted_shipping_address() ) {
			$address_comparison_fields = apply_filters( 'woi_pdf_address_comparison_fields', array(
				'first_name',
				'last_name',
				'company',
				'address_1',
		...
		return apply_filters( 'woi_pdf_ships_to_different_address', false, $order, $this );
	}
	...
	public function get_billing_email() {
		...
		return apply_filters( 'woi_pdf_billing_email', sanitize_email( $billing_email ), $this );
	}
	...
	public function get_phone( $phone_type = 'billing' ) {
		...
		return woi_pdf_sanitize_phone_number( $phone );
	}

	public function get_billing_phone() {
		$phone = $this->get_phone( 'billing' );

		return apply_filters( "woi_pdf_billing_phone", $phone, $this );
	}
	...
	public function get_shipping_phone( $fallback_to_billing = false ) {
		...
		return apply_filters( "woi_pdf_shipping_phone", $phone, $this );
	}
	...
	public function get_shipping_address(): string {
		...
		if ( empty( $address ) ) {
			if (
				apply_filters( 'woi_pdf_shipping_address_fallback', ( 'packing-slip' === $this->get_type() ), $this ) &&
		...
		$address = apply_filters( 'woi_pdf_shipping_address', woi_pdf_sanitize_html_content( $address, 'address' ), $this );
	...
	public function get_custom_field( $field_name ) {
		...
		return apply_filters( 'woi_pdf_billing_custom_field', $custom_field, $this );
	}
		...
		if ( ! empty( $custom_field ) || $display_empty ) {
			...
			if ( is_array( $custom_field ) ) {
				$custom_field = array_map( function( $field ) use ( $allow_tags ) {
					return woi_pdf_sanitize_html_content( $field, 'custom_field', $allow_tags );
				}, $custom_field );
				echo wp_kses( $field_label . implode( '<br>', $custom_field ), $allow_tags );
			} else {
				$custom_field = woi_pdf_sanitize_html_content( $custom_field, 'custom_field', $allow_tags );
	...
	public function order_notes( string $filter = 'customer', bool $include_system_notes = true ): void {
		...
		if ( ! empty( $notes ) ) {
			foreach ( $notes as $note ) {
				...
				$content     = apply_filters( 'woi_pdf_order_note', $content, $note );
				?>
				<div class="<?php echo esc_attr( implode( ' ', $css_class ) ); ?>">
					<?php echo woi_pdf_sanitize_html_content( $content, 'notes' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
				</div>
				<?php
			}
		}
	...
	public function get_payment_method() {
		...
		$payment_method = woi_pdf_dynamic_translate( $payment_method_title, 'woocommerce' );

		return apply_filters( 'woi_pdf_payment_method', $payment_method, $this );
	}
	...
	public function get_shipping_method() {
		$shipping_method = woi_pdf_dynamic_translate( $this->order->get_shipping_method(), 'woocommerce' );
		return apply_filters( 'woi_pdf_shipping_method', $shipping_method, $this );
	}
	...
	public function get_order_number() {
		...
		return apply_filters( 'woi_pdf_order_number', $order_number, $this );
	}
	...
	public function get_order_items(): array {
		...
		if ( sizeof( $items ) > 0 ) {
			foreach ( $items as $item_id => $item ) {
				...
				$data['row_class'] = apply_filters( 'woi_pdf_item_row_class', 'item-' . $item_id, $this->get_type(), $this->order, $item_id );
				...
				$data['name'] = apply_filters( 'woi_pdf_order_item_name', $data['name'], $item, $this->order );
		...
		return apply_filters( 'woi_pdf_order_items_data', $data_list, $this->order, $this->get_type() );
	}
	...
	public function get_tax_rate( $item, $order, $force_calculation = false ) {
		...
		if ( ! apply_filters( 'woi_pdf_calculate_tax_rate', false ) ) {
			$tax = new \WC_Tax();
	...
	public function get_thumbnail( $product ) {
		...
		$size                  = apply_filters( 'woi_pdf_thumbnail_size', $thumbnail_size );
		...
		if ( apply_filters( 'woi_pdf_use_path', true ) && ! WOI_PDF()->file_system->exists( $thumbnail_path ) ) {
		...
		} elseif ( apply_filters( 'woi_pdf_use_path', true ) && ! WOI_PDF()->file_system->exists( $thumbnail_path ) ) {
	...
	public function get_woocommerce_totals() {
		...
		$totals = apply_filters( 'woi_pdf_raw_order_totals', $this->order->get_order_item_totals(), $this->order );
		...
		if ( ! $this->is_refund( $this->order ) && apply_filters( 'woi_pdf_remove_refund_totals', true, $this ) ) {
		...
		return apply_filters( 'woi_pdf_woocommerce_totals', $totals, $this->order, $this->get_type() );
	}
	...
	public function get_order_subtotal( $tax = 'excl', $discount = 'incl' ) { // set $tax to 'incl' to include tax, same for $discount
		...
		return apply_filters( 'woi_pdf_order_subtotal', $subtotal, $tax, $discount, $this );
	}
	...
	public function get_order_shipping( $tax = 'excl' ) { // set $tax to 'incl' to include tax
		...
		return apply_filters( 'woi_pdf_order_shipping', $shipping, $tax, $this );
	}
	...
	public function get_order_discount( $type = 'total', $tax = 'incl' ) {
		...
		if ( round( $discount_value, 3 ) != 0 ) {
			return apply_filters( 'woi_pdf_order_discount', $discount, $type, $tax, $this );
		}
	}
	...
	public function get_order_taxes() {
		...
		if ( $order_taxes = $this->order->get_taxes() ) {
			...
			return apply_filters( 'woi_pdf_order_taxes', $taxes, $this );
		}
	}
	...
	public function get_order_grand_total( $tax = 'incl' ) {
		...
		return apply_filters( 'woi_pdf_order_grand_total', $grand_total, $tax, $this );
	}
	...
	public function get_shipping_notes(): string {
		...
		if ( apply_filters( 'woi_pdf_shipping_notes_strip_all_tags', false ) ) {
			$shipping_notes = wp_strip_all_tags( $shipping_notes );
		}

		return apply_filters( 'woi_pdf_shipping_notes', $shipping_notes, $this );
	}
	...
	public function shipping_notes(): void {
		...
		if ( ! empty( $shipping_notes ) ) {
			echo woi_pdf_sanitize_html_content( $shipping_notes, 'notes' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	}
	...
	public function get_document_notes(): string {
		...
		return apply_filters( 'woi_pdf_document_notes', $document_notes ?? '', $this );
	}
	...
	public function document_notes(): void {
		...
		if ( ! empty( $document_notes ) ) {
			echo woi_pdf_sanitize_html_content( wpautop( $document_notes ), 'notes' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	}
