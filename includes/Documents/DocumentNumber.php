...
if ( ! class_exists( '\\WOI\\PDF\\Documents\\DocumentNumber' ) ) :
...
class DocumentNumber {
	...
	public function __construct( $number, array $settings = array(), ?OrderDocument $document = null, ?\WC_Abstract_Order $order = null ) {
		$number = apply_filters( 'woi_pdf_raw_document_number', $number, $settings, $document, $order );
		...
		} elseif ( ! isset( $this->formatted_number ) && isset( $this->number ) ) {
			// Format inline when no document context is available (e.g. loading stored number data).
			$padding = $this->padding ?? 0;
			$prefix  = $this->prefix ?? '';
			$suffix  = $this->suffix ?? '';
			$plain   = (string) $this->number;

			if ( $padding > 0 ) {
				$plain = str_pad( $plain, $padding, '0', STR_PAD_LEFT );
			}

			$this->formatted_number = apply_filters(
				'woi_pdf_format_document_number',
				$prefix . $plain . $suffix,
				$this,
				null,
				null
	...
	public function load_number_data( array $data ): void {
		$numeric_properties = apply_filters(
			'woi_pdf_document_number_numeric_properties',
	...
	public function get_formatted(): string {
		...
		return apply_filters( 'woi_pdf_formatted_document_number', $formatted_number, $this, $this->document_type, $this->order_id );
	}
	...
	public function apply_formatting( OrderDocument $document, \WC_Abstract_Order $order ): string {
		$formatted_number = woi_pdf_format_document_number( $this->get_plain(), $this->get_prefix(), $this->get_suffix(), $this->get_padding(), $document, $order );
