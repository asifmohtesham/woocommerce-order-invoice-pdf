...
if ( ! class_exists( '\\WOI\\PDF\\Frontend' ) ) :
...
class Frontend {
	...
	public function __construct() {
		...
		// Shortcodes
		add_shortcode( 'woi_pdf_download_invoice', array( $this, 'generate_document_shortcode' ) );
		add_shortcode( 'woi_pdf_download_pdf', array( $this, 'generate_document_shortcode' ) );
		add_shortcode( 'woi_pdf_document_link', array( $this, 'generate_document_shortcode' ) );
	...
	public function my_account_invoice_actions( array $actions, \WC_Abstract_Order $order ): array {
		...
		$document_title = __( 'Invoice', 'woocommerce-orders-invoice-pdf' );
		$invoice        = woi_pdf_get_document( $document_type, $order );

		if ( ! $invoice ) {
			return apply_filters( 'woi_pdf_myaccount_actions', $actions, $order );
		}
		...
		if ( ! $invoice_allowed ) {
			$invoice_allowed = in_array(
				$order->get_status(),
				apply_filters( 'woi_pdf_myaccount_allowed_order_statuses', array() ),
				true
			);
		}
		...
		if ( $invoice_allowed ) {
			...
			$endpoint_instance = WOI_PDF()->endpoint;

			$actions[ $document_type ] = array(
				'url'  => $endpoint_instance->get_document_link( $order, $document_type, array( 'my-account' => 'true' ) ),
				'name' => apply_filters( 'woi_pdf_myaccount_button_text', $name, $invoice ),
			);

			if ( $invoice->is_enabled( 'xml' ) && woi_pdf_edi_is_available() ) {
				$actions[ $document_type . '_xml' ] = array(
					...
					'name' => apply_filters( 'woi_pdf_myaccount_button_text', "E-{$name}", $invoice ),
				);
			}
		}
	...
	public function open_my_account_link_on_new_tab(): void {
		...
		if ( $is_account || $is_order_received ) {
			...
			if ( isset( $general_settings['download_display'] ) && 'display' === $general_settings['download_display'] ) {
				...
				$file_path = WOI_PDF()->plugin_path() . '/assets/js/my-account-link' . $suffix . '.js';

				if ( WOI_PDF()->file_system->exists( $file_path ) ) {
					$script = WOI_PDF()->file_system->get_contents( $file_path );

					if ( $script && WOI_PDF()->endpoint->pretty_links_enabled() ) {
						$script = str_replace( 'generate_woi_pdf', WOI_PDF()->endpoint->get_identifier(), $script );
					}
	...
	public function add_invoice_number_to_wc_legacy_order_api( array $data, \WC_Abstract_Order $order ): array {
		$data['woi_pdf_invoice_number'] = $this->get_invoice_number( $order );
	...
	public function generate_document_shortcode( array $atts, ?string $content = null, string $shortcode_tag = '' ): string {
			...
			if ( $document->get_type() === $values['document_type'] ) {
				...
				if ( ! empty( $values['link_text'] ) ) {
					...
					$link_text = sprintf(
						...
						__( 'Download %s (PDF)', 'woocommerce-orders-invoice-pdf' ),
		...
		$document = woi_pdf_get_document( $values['document_type'], $order );

		if ( ! $document || ! $document->is_allowed() ) {
			return '';
		}

		$pdf_url = WOI_PDF()->endpoint->get_document_link( $order, $values['document_type'], [ 'shortcode' => 'true' ] );

		if ( 'woi_pdf_document_link' === $shortcode_tag ) {
	...
	public function disable_storing_document_settings(): void {
		add_filter( 'woi_pdf_document_store_settings', '__return_false', 9999 );
	}
	...
	public function restore_storing_document_settings(): void {
		remove_filter( 'woi_pdf_document_store_settings', '__return_false', 9999 );
	}
	...
	public function checkout_field_display_checkout_block_field(): void {
		...
		$args = array(
			...
			'sanitize_callback' => static function ( $val ) {
				...
				return (string) apply_filters( 'woi_pdf_checkout_field_sanitize', $val );
			},
			'validate_callback' => function ( $val ) {
				...
				if ( ! $this->checkout_field_is_vat_number() ) {
					$result = apply_filters( 'woi_pdf_checkout_field_validate', true, $val );
					return ( $result instanceof \WP_Error ) ? $result : true;
				}

				$result = apply_filters( 'woi_pdf_checkout_field_validate', $this->checkout_field_validate_vat_number_value( $val ), $val );
				return ( $result instanceof \WP_Error ) ? $result : true;
			},
		);

		$args = apply_filters( 'woi_pdf_checkout_field_block_args', $args );

		woi_pdf_register_additional_checkout_field( $args );
	}
	...
	public function checkout_field_set_checkout_block_field_value(): void {
		...
		add_filter(
			...
			static function ( $value, string $group, \WC_Data $wc_object ) {
				...
				$stored = (string) get_user_meta( $user_id, 'woi_pdf_checkout_field', true );

				return (string) apply_filters( 'woi_pdf_checkout_field_default_value', $stored, $value, $group, $wc_object );
			},
			10,
			3
		);
	...
	public function checkout_field_save_checkout_block_field( string $key, $value, string $group, object $wc_object ): void {
		...
		$val = (string) apply_filters( 'woi_pdf_checkout_field_sanitize', $val );

		// Save on order.
		$order_meta_key = '_woi_pdf_checkout_field';
		...
		if ( $customer_id > 0 ) {
			if ( '' === trim( $val ) ) {
				delete_user_meta( $customer_id, 'woi_pdf_checkout_field' );
			} else {
				update_user_meta( $customer_id, 'woi_pdf_checkout_field', $val );
			}
		}
	}
	...
	public function checkout_field_display_classic_checkout_field( $fields ): array {
		...
		$fields['order'][ $key ] = apply_filters( 'woi_pdf_checkout_field_classic_args', $args );
	...
	public function checkout_field_set_classic_checkout_field_value( $value, string $input ) {
		if ( 'woi_pdf_checkout_field' !== $input ) {
			return $value;
		}
		...
		return (string) apply_filters( 'woi_pdf_checkout_field_default_value', $stored, $value, 'classic', null );
	}
	...
	public function checkout_field_display_admin_billing( \WC_Order $order ): void {
		...
		$value = (string) $order->get_meta( '_woi_pdf_checkout_field', true );
		$value = trim( $value );
	...
	public function account_details_display_checkout_field(): void {
		...
		$value = (string) apply_filters( 'woi_pdf_checkout_field_default_value', $value, $value, 'my-account', null );

		$label       = $this->checkout_field_get_label();
		$description = '';

		if ( $this->checkout_field_is_vat_number() ) {
			$description = __( 'Please include the country prefix (for example NL123456789).', 'woocommerce-orders-invoice-pdf' );
		}
	...
	public function checkout_field_is_vat_number(): bool {
		...
		if ( \WOI_PDF()->vat_plugins->has_active() ) {
			return false;
		}
	...
	private function checkout_field_get_label(): string {
		...
		return (string) apply_filters( 'woi_pdf_checkout_field_label', $label );
	}
	...
	private function checkout_field_validate_vat_number_value( string $raw_value ) {
		 ...
		 * @param string $raw_value
		 */
		$vat = (string) apply_filters( 'woi_pdf_checkout_field_vat_normalize', $vat, $raw_value );

		// Must start with a valid country prefix.
		if ( ! woi_pdf_edi_vat_number_has_country_prefix( $vat ) ) {
			$error = new \WP_Error(
				'invalid_vat_prefix',
				__( 'Please enter a VAT number including a valid country prefix (for example PT123456789).', 'woocommerce-orders-invoice-pdf' )
			);
			 ...
			 * @param string   $vat
			 */
			return apply_filters( 'woi_pdf_checkout_field_vat_prefix_error', $error, $vat );
		}
		 ...
		 * @param string         $raw_value
		 */
		return apply_filters( 'woi_pdf_checkout_field_vat_validate', true, $vat, $raw_value );
	}
